...
static int mtk_pcm_hdmi_start(struct snd_pcm_substream *substream)
{
    ...
    kal_uint32 volatile u4tmpValue2;
     ...
     printk("[mtk_pcm_hdmi_start] runtime->rate = %d, runtime->channels = %d, runtime->period_size = %d\n", runtime->rate, runtime->channels, (unsigned int)runtime->period_size);
    ...
    {
        printk("[mtk_pcm_hdmi_start]  32bit!\n");
        SetMemIfFetchFormatPerSample(Soc_Aud_Digital_Block_MEM_HDMI, AFE_WLEN_32_BIT_ALIGN_24BIT_DATA_8BIT_0); 
    ...
    printk("[mtk_pcm_hdmi_start] AFE_IRQ_MCU_STATUS =0x%x AFE_IRQ_MCU_EN= 0x%x, AFE_IRQ_CNT5=0x%x, AFE_IRQ_DEBUG =0x%x\n", Afe_Get_Reg(AFE_IRQ_MCU_STATUS), Afe_Get_Reg(AFE_IRQ_MCU_EN), Afe_Get_Reg(AFE_IRQ_CNT5), Afe_Get_Reg(AFE_IRQ_DEBUG));
