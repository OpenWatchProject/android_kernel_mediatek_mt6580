...
static bool TurnOnVOWDigitalHW(bool enable)
{
	...
	if (enable) {
		...
		/* Ana_Set_Reg(INT_CON0, 0x0015, 0x0800);
		//disable VOW interrupt. BIT11 */
#ifdef VOW_STANDALONE_CONTROL
		/* move to vow driver */
...
static bool TurnOnVOWADcPowerACC(int MicType, bool enable)
{
	...
	pr_warn("%s MicType = %d enable = %d, mIsVOWOn=%d, mAudio_VOW_Mic_type=%d\n", __func__,
	       MicType, enable, mIsVOWOn, mAudio_VOW_Mic_type);
	...
	if (enable) {
		...
		case AUDIO_VOW_MIC_TYPE_Headset_MIC:
			...
			/* VA28 remote sense, only set bit9, RG_VAUD28_SENSE_SEL */
			Ana_Set_Reg(LDO_CON2, 0x8103, 0x000f);
			/* LDO enable control by RG_VAUD28_EN, Enable AVDD28_LDO (Default on),
			LPW control by VAUD28_MODE_SET Enable low power mode. bit0~3 */
			Ana_Set_Reg(AUDENC_ANA_CON2, 0x0003, 0x0003);
			/* Enable audio uplink VOW LPW globe bias, Enable audio uplink LPW mode. bit0~1 */
			Ana_Set_Reg(AUDENC_ANA_CON12, 0x0409, 0x040D);
			/* Enable fbdiv relatch (low jitter), Set DCKO = 1/4 F_PLL, Enable VOWPLL CLK. bit:0,2,3,10 */
			Ana_Set_Reg(AUDENC_ANA_CON14, 0x0000, 0x0038);
			/* PLL VCOBAND. bit5:3 */
			Ana_Set_Reg(AUDENC_ANA_CON12, 0x06F9, 0x03f0);
			/* PLL devider ratio. bit9:4 */
			Ana_Set_Reg(AUDENC_ANA_CON13, 0x8180, 0x8000);
			/* PLL low power. bit:15 */
			Ana_Set_Reg(AUDENC_ANA_CON3, 0x0009, 0x000D);
			/* ADC CLK from VOWPLL (12.85/4MHz), Enable Audio ADC FBDAC 0.25FS LPW. BIT0,2,3 */
			Ana_Set_Reg(AUDDEC_ANA_CON6, 0x0100, 0x0100);
			/* Enable  LCLDO_ENC 1P8V. BIT8 */
			Ana_Set_Reg(AUDDEC_ANA_CON7, 0x0006, 0x0006);
			/* LCLDO_ENC remote sense.BIT1,2 */
			...
			/* Enable MICBIAS1 lowpower mode, MISBIAS1 = 2P5V.BIT4,10~8,11 */
			Ana_Set_Reg(AUDENC_ANA_CON0, 0x0081, 0x00C1);
			/* Audio L preamplifier input sel : AIN0, Enable audio L PGA. BIT0,7:6 */
			Ana_Set_Reg(AUDENC_ANA_CON15, 0x0003, 0x0007);
			/* Audio L PGA 18 dB gain. BIT:2:0 */
			Ana_Set_Reg(AUDENC_ANA_CON3, 0x0809, 0xf900);
			/* PGA stb enhance. BIT:8,15:11 */
			Ana_Set_Reg(AUDENC_ANA_CON0, 0x0581, 0x0500);
			/* Audio L ADC input sel : L PGA, Enable audio L ADC.BIT:8,9,10 */
		...
		case AUDIO_VOW_MIC_TYPE_Handset_DMIC_800K:
			...
			/* Enable audio uplink VOW LPW globe bias. BIT1 */
			Ana_Set_Reg(AUDENC_ANA_CON9, 0x0A01, 0x0F01);
			/* Enable MICBIAS0 lowpower mode, MISBIAS0 = 1P9V..BIT0,10~8,11 */
...
/* GPIO25:  mode 2 */
		...
		case AUDIO_VOW_MIC_TYPE_Handset_AMIC_DCCECM:
			...
			/* Audio L preamplifier DCC precharge.BIT2 */
			Ana_Set_Reg(AUDENC_ANA_CON0, 0x0045, 0x00C1);
			/* Audio L preamplifier input sel : AIN0,Enable audio L PGA.BIT0,6,7 */
			Ana_Set_Reg(AUDENC_ANA_CON15, 0x0003, 0x0007);
			/* Audio L PGA 18 dB gain.BIT0~2 */
			Ana_Set_Reg(AUDENC_ANA_CON0, 0x0047, 0x0002);
			/* Audio L preamplifier DCCEN.BIT1 */
			Ana_Set_Reg(AUDENC_ANA_CON3, 0x0809, 0xf800);
			/* PGA stb enhance. BIT11~15 */
			Ana_Set_Reg(AUDENC_ANA_CON0, 0x0547, 0x0700);
			/* Audio L ADC input sel : L PGA, Enable audio L ADC.BIT8,9,10 */
			/* delay 100us */
			usleep(1*1000);
			Ana_Set_Reg(AUDENC_ANA_CON0, 0x0543, 0x0004);
			/* Audio L preamplifier DCC precharge off. BIT2=0 */
		...
		case AUDIO_VOW_MIC_TYPE_Headset_MIC_DCCECM:
			...
			/* LCLDO_ENC remote sense */
			Ana_Set_Reg(AUDENC_ANA_CON9, 0x0F10, 0x0f10);
			/* Enable MICBIAS1 lowpower mode, MISBIAS1 = 2P5V.BIT:4,8:11 */

			if (MicType == AUDIO_VOW_MIC_TYPE_Headset_MIC_DCCECM) {
				Ana_Set_Reg(AUDENC_ANA_CON9, 0x0F20, 0x0060);
				/* MICBIAS1 DCC ECM single SwithP/N on(P:1,N:0). BIT5,6 */
			}
			/* Ana_Set_Reg(AUDENC_ANA_CON9, 0x0F70, 0xffff);
			//MICBIAS1 DCC SwithP/N on */
			Ana_Set_Reg(AUDENC_ANA_CON0, 0x0004, 0x0004);
			/* Audio L preamplifier DCC precharge.BIT2 */
			Ana_Set_Reg(AUDENC_ANA_CON0, 0x0045, 0x00C1);
			/* Audio L preamplifier input sel : AIN0,Enable audio L PGA.BIT0,6,7 */
		...
		case AUDIO_VOW_MIC_TYPE_Headset_MIC:
			...
			/* Audio L preamplifier input sel : off, disable audio L PGA. BIT:0,6,7 */
			Ana_Set_Reg(AUDENC_ANA_CON0, 0x0000, 0x0700);
			/* Audio L ADC input sel : off, disable audio L ADC.BIT:8,9,10 */
			Ana_Set_Reg(AUDENC_ANA_CON3, 0x0009, 0xf900);
			/* PGA stb enhance off,BIT:8,15:11 */
			Ana_Set_Reg(AUDENC_ANA_CON15, 0x0000, 0x0007);
			/* Audio L PGA 0 dB gain,BIT:0~2 */
			Ana_Set_Reg(AUDENC_ANA_CON4, 0x0000, 0x0800);
			/* BIT11 */
			Ana_Set_Reg(AUDENC_ANA_CON6, 0x2020, 0xffff);
			Ana_Set_Reg(AUDENC_ANA_CON9, 0x0000, 0x0f01);
			/* disable MICBIAS0 lowpower mode, MISBIAS0 = 1P7V. BIT:0,8:10,11 */
			Ana_Set_Reg(AUDDEC_ANA_CON7, 0x0000, 0x0006);
			/* LCLDO_ENC remote sense off, BIT:1,2 */
			Ana_Set_Reg(AUDDEC_ANA_CON6, 0x0000, 0x0100);
			/* disable  LCLDO_ENC 1P8V */
			Ana_Set_Reg(AUDENC_ANA_CON3, 0x0000, 0x000D);
			/* ADC CLK from VOWPLL (12.85/4MHz) off,
			Enable Audio ADC FBDAC 0.25FS LPW off. BIT:0,2,3 */
			Ana_Set_Reg(AUDENC_ANA_CON13, 0x0180, 0x8000);
			/* PLL low power off. BIT:15 */
			Ana_Set_Reg(AUDENC_ANA_CON12, 0x02F0, 0x040D);
			/* disable fbdiv relatch (low jitter),Set DCKO = 1/4 F_PLL off,
			disable VOWPLL CLK. BIT:0,2,3,10 */
			Ana_Set_Reg(AUDENC_ANA_CON2, 0x0000, 0x0003);
			/* disable audio uplink VOW LPW globe bias,
			disable audio uplink LPW mode.BIT:0,1 */
			...
			/* LDO enable control by RG_VAUD28_EN, Enable AVDD28_LDO (Default on),
			LPW control by VAUD28_MODE_SET, disable low power mode.BIT:0~3 */
		...
		case AUDIO_VOW_MIC_TYPE_Handset_DMIC_800K:
			...
			/* DMIC disable.BIT0,1,2 */
			Ana_Set_Reg(AUDENC_ANA_CON9, 0x0800, 0x0701);
			/* disable MICBIAS0 lowpower mode, MISBIAS0 = 1P7V.BIT0,8~10 */
			Ana_Set_Reg(AUDENC_ANA_CON2, 0x0000, 0x0002);
			/* disable audio uplink VOW LPW globe bias.BIT1 */
			/* no need to config this reg due to VOW no need */
			/* NvregEnable(true); //enable audio globe bias (Default on) */
			Ana_Set_Reg(AUDENC_ANA_CON13, 0x0180, 0x8000);
			/* PLL low power off.BIT:15 */
			Ana_Set_Reg(AUDENC_ANA_CON12, 0x02F0, 0x040D);
			/* disable fbdiv relatch (low jitter),Set DCKO = 1/4 F_PLL off,
			disable VOWPLL CLK.BIT0,2,3,10 */
			Ana_Set_Reg(LDO_CON2, 0x8102, 0x000f);
			/* LDO enable control by RG_VAUD28_EN, Enable AVDD28_LDO (Default on),
			LPW control by VAUD28_MODE_SET, disable low power mode.BIT:0~3 */
		...
		case AUDIO_VOW_MIC_TYPE_Handset_AMIC_DCCECM:
			...
			/* Audio L preamplifier input sel : off, disable audio L PGA.BIT0,6,7 */
			Ana_Set_Reg(AUDENC_ANA_CON0, 0x0000, 0x0700);
			/* Audio L ADC input sel : off, disable audio L ADC.BIT:8,9,10 */
			Ana_Set_Reg(AUDENC_ANA_CON3, 0x0009, 0xf800);
			/* PGA stb enhance off.BIT11~15 */
			Ana_Set_Reg(AUDENC_ANA_CON15, 0x0000, 0x0007);
			/* Audio L PGA 0 dB gain.BIT0~2 */
			Ana_Set_Reg(AUDENC_ANA_CON0, 0x0000, 0x0002);
			/* Audio L preamplifier DCCEN.BIT1 */
			Ana_Set_Reg(AUDENC_ANA_CON9, 0x0A01, 0x0006);
			/* MICBIAS0 DCC SwithP/N on.BIT1,2 */
			Ana_Set_Reg(AUDENC_ANA_CON4, 0x0000, 0x0800);
			/* BIT11 */
			Ana_Set_Reg(AUDENC_ANA_CON6, 0x2020, 0xffff);
			Ana_Set_Reg(AUDENC_ANA_CON9, 0x0000, 0x0f01);
			...
			disable VOWPLL CLK. BIT:0,2,3,10 */
			Ana_Set_Reg(AUDENC_ANA_CON2, 0x0000, 0x0003);
			/* disable audio uplink VOW LPW globe bias, disable audio uplink LPW mode.BIT:0,1 */
			/* no need to config this reg due to VOW no need */
			/* NvregEnable(true); //enable audio globe bias (Default on) */
			Ana_Set_Reg(LDO_CON2, 0x8102, 0x000f);
			/* LDO enable control by RG_VAUD28_EN, Enable AVDD28_LDO (Default on),
		...
		case AUDIO_VOW_MIC_TYPE_Headset_MIC_DCCECM:
			...
			/* MICBIAS1 DCC SwithP/N on. BIT5,6 */
			Ana_Set_Reg(AUDENC_ANA_CON4, 0x0000, 0x0800);
			/* BIT11 */
			Ana_Set_Reg(AUDENC_ANA_CON6, 0x2020, 0xffff);
			Ana_Set_Reg(AUDENC_ANA_CON9, 0x0000, 0x0f10);
			/* disable MICBIAS0 lowpower mode, MISBIAS0 = 1P7V.BIT4,8:11 */
			Ana_Set_Reg(AUDDEC_ANA_CON7, 0x0000, 0x0006);
			/* LCLDO_ENC remote sense off, BIT:1,2 */
			Ana_Set_Reg(AUDDEC_ANA_CON6, 0x0000, 0x0100);
			/* disable  LCLDO_ENC 1P8V */
