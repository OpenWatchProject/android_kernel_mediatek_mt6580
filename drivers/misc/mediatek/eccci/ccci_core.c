...
static void ccci_dump_log_rec(struct ccci_modem *md, struct ccci_log *log)
{
	...
	if(!log->droped) {
		printk("[CCCI%d-DUMP]%08X %08X %08X %08X  %5lu.%06lu\n",
			md->index+1,log->msg.data[0],log->msg.data[1],*(((u32 *)&log->msg)+2),log->msg.reserved,(unsigned long)ts_nsec, rem_nsec/1000);
	} else {
		printk("[CCCI%d-DUMP]%08X %08X %08X %08X  %5lu.%06lu -\n",
			md->index+1,log->msg.data[0],log->msg.data[1],*(((u32 *)&log->msg)+2),log->msg.reserved,(unsigned long)ts_nsec, rem_nsec/1000);
	}
