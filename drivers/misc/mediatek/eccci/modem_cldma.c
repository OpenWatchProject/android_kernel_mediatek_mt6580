...
#else // if delayed work and timer are both not used
...
#define IS_NET_QUE(md,qno) ((md->md_state!=EXCEPTION || md->ex_stage!=EX_INIT_DONE) &&((1<<qno) & NET_RX_QUEUE_MASK)) 
/*
...
static void cldma_dump_gpd_queue(struct ccci_modem *md, unsigned int qno)
{
	...
	req = list_entry(md_ctrl->txq[qno].tr_ring, struct ccci_request, entry);
	cldma_dump_gpd_ring(md->index, req->gpd_addr, tx_queue_buffer_number[qno]);

	printk("[CCCI%d/CLDMA] dump rxq%d GPD\n", md->index+1, qno);
	req = list_entry(md_ctrl->rxq[qno].tr_ring, struct ccci_request, entry);
	cldma_dump_gpd_ring(md->index, req->gpd_addr, rx_queue_buffer_number[qno]);
}
...
{
	struct md_cd_ctrl *md_ctrl = (struct md_cd_ctrl *)md->private_data;
	printk("[CCCI%d-DUMP]Current txq%d pos: tr_done=%x, tx_xmit=%x\n",md->index+1, qno,(unsigned int)md_ctrl->txq[qno].tr_done->gpd_addr, (unsigned int)md_ctrl->txq[qno].tx_xmit->gpd_addr);
	printk("[CCCI%d-DUMP]Current rxq%d pos: tr_done=%x\n",md->index+1, qno,(unsigned int)md_ctrl->rxq[qno].tr_done->gpd_addr);
...
fill_and_move:
		...
		if(count++ >= budget) {
		   *result = REACH_BUDGET;
			if(IS_NET_QUE(md, queue->index) && md->capability&MODEM_CAP_NAPI) {
				break;
		    }
		}
...
again:
	count = cldma_rx_collect(queue, queue->budget, !IS_NET_QUE(md,queue->index), &result, &rx_bytes);
//Trace end
#ifdef CLDMA_TRACE	
	if(count){
...
static int cldma_tx_collect(struct md_cd_queue *queue, int budget, int blocking, int *result)
{
	...
	while(1) {
		...
		// restore IOC setting
		if(!IS_NET_QUE(md,queue->index) && (req->ioc_override & 0x80)) {
...
static enum hrtimer_restart cldma_tx_done_timer(struct hrtimer *timer)
{
	...
	if(IS_NET_QUE(md,queue->index)){
		CCCI_DBG_MSG(md->index, TAG, "qno %d, cldma_tx_done_timer: %d,%llu,cnt=%d\n", queue->index,count,md_ctrl->tx_done_last_start_time[queue->index],count);
	}
...
{
#ifdef CLDMA_TIMER_LOOP
	struct md_cd_queue *queue = container_of(work, struct md_cd_queue, cldma_tx_work);
#else
...
//Trace end
...
#ifndef CLDMA_TIMER_LOOP
	if(count) {
		ret = queue_delayed_work(queue->worker, &queue->cldma_tx_work, msecs_to_jiffies(10));
	} else
#endif
	{
	...
	if(IS_NET_QUE(md,queue->index)){
		CCCI_DBG_MSG(md->index, TAG, "qno%d, cldma_tx_done: %d,%llu, count=%d, ret=%d\n", queue->index,count,md_ctrl->tx_done_last_start_time[queue->index],count,ret);
	}
}
...
static void cldma_tx_queue_init(struct md_cd_queue *queue)
{
	...
	INIT_WORK(&queue->cldma_tx_work, cldma_tx_done);
#else
		...
		for(i=0; i<QUEUE_LEN(md_ctrl->txq); i++) {
			if(L2TISAR0 & CLDMA_BM_INT_DONE & (1<<i)) {
				...
				if (IS_NET_QUE(md, i) && (hrtimer_active(&md_ctrl->txq[i].cldma_poll_timer) == 0)) {
					...
					ret = queue_work(md_ctrl->txq[i].worker, &md_ctrl->txq[i].cldma_tx_work);
		...
		for(i=0; i<QUEUE_LEN(md_ctrl->rxq); i++) {
			if(L2RISAR0 & CLDMA_BM_INT_DONE & (1<<i)) {
				...
				if(md->md_state!=EXCEPTION && md_ctrl->rxq[i].napi_port) {
					...
					ret = queue_work(md_ctrl->rxq[i].worker, &md_ctrl->rxq[i].cldma_rx_work);
				}
...
#else
				ret = queue_delayed_work(md_ctrl->txq[i].worker, &md_ctrl->txq[i].cldma_tx_work, 0);
				CCCI_DBG_MSG(md->index, TAG, "qno%d queue_delayed_work=%d\n", i, ret);
#endif
#endif
			}
...
* So we should check current port no whethe is network queue by IS_NET_QUE(md,qno),
...
retry:
	...
	if(queue->free_slot > 0) {
		...
		md_cd_lock_cldma_clock_src(0);
#ifdef CLDMA_NO_TX_IRQ
		cldma_tx_collect(queue, 0, 0, &ret);	
#endif
		if(blocking) {
