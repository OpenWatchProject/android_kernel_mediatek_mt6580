					 ...
					 int buf_size, int rx_size, int tx_size)
{
	...
	CCCI_INF_MSG(md_id, TAG,
		     ...
		     buf, buf_size, buflen, rx_size, tx_size,
		     CCCI_RINGBUF_CTL_LEN);
	if (buf_size < buflen)
		return NULL;
		       ...
		       unsigned char *data, int data_len)
{
	...
	CCCI_DBG_MSG(md_id, TAG,
		     ...
		     data_len, aligned_data_len, 16, length,
		     ringbuf->tx_control.read);
...
int ccci_ringbuf_readable(int md_id, struct ccci_ringbuf *ringbuf)
{
	...
	if (header[0] != CCIF_PKG_HEADER) {
		...
		CCCI_ERR_MSG(md_id, TAG,
			     ...
			     rx_buffer, header[0]);
		ccci_ringbuf_dump(md_id, "readable", rx_buffer, read, length,
				  size);
