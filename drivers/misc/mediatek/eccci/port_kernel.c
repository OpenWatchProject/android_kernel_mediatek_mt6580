...
static int get_eint_attr_val(struct device_node *node, int index)
{
	...
	for (type = 0; type < SIM_HOT_PLUG_EINT_MAX; type++) {
		...
		case SIM_HOT_PLUG_EINT_SRCPIN:
			ret = of_property_read_u32_index(node, md_eint_struct[type].property, md_eint_struct[type].index, &value);
			break;
		default:
		...
		if (!ret) {
			...
			if(type == SIM_HOT_PLUG_EINT_POLARITY) {
				...
				case IRQ_TYPE_LEVEL_LOW:
					md_eint_struct[SIM_HOT_PLUG_EINT_POLARITY].value_sim[index]= (value&0x5)?1:0; // 1/4-> IRQ_TYPE_EDGE_RISING / IRQ_TYPE_LEVEL_HIGH set 1
					md_eint_struct[SIM_HOT_PLUG_EINT_SENSITIVITY].value_sim[index]= (value&0x3)?1:0; // 1/2-> IRQ_TYPE_EDGE_RISING / IRQ_TYPE_EDGE_FALLING set 1
					break;
				default: // invalid
				...
				//CCCI_INF_MSG(-1, RPC, "%s:  --- %d_%d_%d \n", md_eint_struct[type].property, md_eint_struct[type].index, 
				//md_eint_struct[SIM_HOT_PLUG_EINT_POLARITY].value_sim[index], md_eint_struct[SIM_HOT_PLUG_EINT_SENSITIVITY].value_sim[index]);
				type++;
			} // special case: polarity's position == sensitivity's end]
			else {
				md_eint_struct[type].value_sim[index] = value;
				//CCCI_INF_MSG(-1, RPC, "%s: --- %d_%d \n", md_eint_struct[type].property, md_eint_struct[type].index, md_eint_struct[type].value_sim[index]);
			}
		} else {
        ...
        if (eint_node_prop.ExistFlag &(1<<i)) {
            if (!(strncmp(name, eint_node_prop.name[i].node_name, name_len)))
            {
              ...
              *len = sizeof(sim_value);             
              ...
              //CCCI_INF_MSG(-1, RPC, "get_eint_value type name:%s, sizeof: %d, sim_info: %d, %d\n", 
...
err4:
	...
	case IPC_RPC_USIM2NFC_OP:
	{
		...
		CCCI_INF_MSG(md->index, KERN, "IPC_RPC_USIM2NFC_OP request=%x\n", input->lock_vsim1);
