...
static struct mag_hw cust_mag_hw = {    
	...
	#elif defined(Z49W5)		
		.direction = 5, 
	#elif defined(CONFIG_Z69_PROJECT)		
		.direction = 7, 
	#else    
		.direction =4,   
	#endif    
...
//modified for mmc3416x----changhua start
	...
	if (I2C_TxData(tmp, 2) < 0) {
		printk("mmc 3416 I2C_TxData error 1\n");
		return -1;
	}
	msleep(MMC3416X_DELAY_SET);
	...
	if (I2C_TxData(tmp, 2) < 0) {
			printk("mmc 3416 I2C_TxData error 2\n");
	}
	msleep(1);
	tmp[0] = MMC3416X_REG_CTRL;
	tmp[1] = 0;
	if (I2C_TxData(tmp, 2) < 0) {
		printk("mmc 3416 I2C_TxData error 3\n");
	}
	msleep(MMC3416X_DELAY_SET);
	...
	tmp[1] = MMC3416X_CTRL_REFILL;
	if (I2C_TxData(tmp, 2) < 0) {
		printk("mmc 3416 I2C_TxData error 4\n");
	}
	msleep(MMC3416X_DELAY_RST);
	tmp[0] = MMC3416X_REG_CTRL;
	tmp[1] = MMC3416X_CTRL_RESET;
	if (I2C_TxData(tmp, 2) < 0) {
			printk("mmc 3416 I2C_TxData error 5\n");
	}
	msleep(1);
	tmp[0] = MMC3416X_REG_CTRL;
	tmp[1] = 0;
	if (I2C_TxData(tmp, 2) < 0) {
		printk("mmc 3416 I2C_TxData error 6\n");
	}
	msleep(1);
