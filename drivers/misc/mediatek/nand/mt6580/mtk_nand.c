...
static int mtk_nand_interface_async(void);
...
__aligned(64) u8 rrtry_buffer[16384 + 1280];
...
void mtk_snand_pm_dump_one_record(u32 idx)
{
    ...
    else
    {
        if (g_snand_pm_data[idx].layer == _SNAND_PM_LAYER_DRIVER)   // DRIVER
        {
            ...
            {
                printk("[%d.%d](%d)  ++-DRV RS r=%d, b=%d, p=%d, s=%d, mtd_len=%d, t=%dus, %s\n", g_snand_pm_data[idx].time_s, g_snand_pm_data[idx].time_ns / 1000, idx, g_snand_pm_data[idx].val[0], g_snand_pm_data[idx].val[0] / ((gn_devinfo.blocksize << 10) / gn_devinfo.pagesize), g_snand_pm_data[idx].val[0] % ((gn_devinfo.blocksize << 10) / gn_devinfo.pagesize), g_snand_pm_data[idx].val[1], mtd_len, g_snand_pm_data[idx].duration, mtk_snand_pm_get_part_name(g_snand_pm_data[idx].val[0]));
            }
            else if (g_snand_pm_data[idx].op == _SNAND_PM_OP_READ_SEC_CACHE_HIT)
            {
                printk("[%d.%d](%d)  ++-DRV RSC r=%d, b=%d, p=%d, s=%d, mtd_len=%d, %s\n", g_snand_pm_data[idx].time_s, g_snand_pm_data[idx].time_ns / 1000, idx, g_snand_pm_data[idx].val[0], g_snand_pm_data[idx].val[0] / ((gn_devinfo.blocksize << 10) / gn_devinfo.pagesize), g_snand_pm_data[idx].val[0] % ((gn_devinfo.blocksize << 10) / gn_devinfo.pagesize), g_snand_pm_data[idx].val[1], mtd_len, mtk_snand_pm_get_part_name(g_snand_pm_data[idx].val[0]));
            }
            else if (g_snand_pm_data[idx].op == _SNAND_PM_OP_READ_PAGE)
            {
                printk("[%d.%d](%d)  ++-DRV RP r=%d, b=%d, p=%d, mtd_len=%d, t=%dus, %s\n", g_snand_pm_data[idx].time_s, g_snand_pm_data[idx].time_ns / 1000, idx, g_snand_pm_data[idx].val[0], g_snand_pm_data[idx].val[0] / ((gn_devinfo.blocksize << 10) / gn_devinfo.pagesize), g_snand_pm_data[idx].val[0] % ((gn_devinfo.blocksize << 10) / gn_devinfo.pagesize), mtd_len, g_snand_pm_data[idx].duration, mtk_snand_pm_get_part_name(g_snand_pm_data[idx].val[0]));
            }
            else if (g_snand_pm_data[idx].op == _SNAND_PM_OP_READ_PAGE_CACHE_HIT)
            {
                printk("[%d.%d](%d)  ++-DRV RPC r=%d, b=%d, p=%d, mtd_len=%d, %s\n", g_snand_pm_data[idx].time_s, g_snand_pm_data[idx].time_ns / 1000, idx, g_snand_pm_data[idx].val[0], g_snand_pm_data[idx].val[0] / ((gn_devinfo.blocksize << 10) / gn_devinfo.pagesize), g_snand_pm_data[idx].val[0] % ((gn_devinfo.blocksize << 10) / gn_devinfo.pagesize), mtd_len, mtk_snand_pm_get_part_name(g_snand_pm_data[idx].val[0]));
            }
            else if (g_snand_pm_data[idx].op == _SNAND_PM_OP_PROGRAM)
            {
                printk("[%d.%d](%d)  ++-DRV WP r=%d, b=%d, t=%dus, %s\n", g_snand_pm_data[idx].time_s, g_snand_pm_data[idx].time_ns / 1000, idx, g_snand_pm_data[idx].val[0], g_snand_pm_data[idx].val[0] / ((gn_devinfo.blocksize << 10) / gn_devinfo.pagesize), g_snand_pm_data[idx].duration, mtk_snand_pm_get_part_name(g_snand_pm_data[idx].val[0]));
            }
        ...
        else    // MTD
        {
            ...
            {
                printk("[%d.%d](%d) +++MTD R from=%d, b=%d, p=%d, col=%d, len=%d\n", g_snand_pm_data[idx].time_s, g_snand_pm_data[idx].time_ns / 1000, idx, g_snand_pm_data[idx].val[0], g_snand_pm_data[idx].val[0] / (gn_devinfo.blocksize << 10), (g_snand_pm_data[idx].val[0] % (gn_devinfo.blocksize << 10)) / gn_devinfo.pagesize, g_snand_pm_data[idx].val[0] % gn_devinfo.pagesize, g_snand_pm_data[idx].val[1]);
            }
...
u32 mtk_nand_page_transform(struct mtd_info *mtd, struct nand_chip *chip, u32 page, u32* blk, u32* map_blk)
{
	...
	if(translate == TRUE)
	{
		...
		if(gn_devinfo.vendor != VEND_NONE)
		{
			//page_in_block = gn_devinfo.feature_set.PairPage[page_in_block];
...
{
	if(gn_devinfo.vendor == VEND_SANDISK || gn_devinfo.vendor == VEND_TOSHIBA|| gn_devinfo.vendor == VEND_HYNIX)
	{
...
{
	if (gn_devinfo.iowidth == IO_ONFI || gn_devinfo.iowidth == IO_TOGGLEDDR || gn_devinfo.iowidth == IO_TOGGLESDR) {
...
static bool mtk_nand_check_dececc_done(u32 u4SecNum)
{
		...
		if (timeval_compare(&timer_cur, &timer_timeout) >= 0)
	      {
			MSG(INIT, "ECC_DECDONE: timeout 0x%x %d time sec %d, usec %d\n", \
				DRV_Reg(ECC_DECDONE_REG16), u4SecNum, (int)(timer_cur.tv_sec-timer_timeout.tv_sec), \
    ...
    while (DRV_Reg32(ECC_DECFSM_REG32) != ECC_DECFSM_IDLE)
    {
		...
		if (timeval_compare(&timer_cur, &timer_timeout) >= 0)
	      {
				MSG(INIT, "ECC_DECDONE: timeout ECC_DECFSM_REG32 0x%x 0x%x %d  time sec %d, usec %d\n", \
...
#define READ_RETRY_STEP (gn_devinfo.feature_set.FeatureSet.readRetryCnt + gn_devinfo.feature_set.FeatureSet.readRetryStart)  // 8 step or 12 step to fix read retry table
...
static void mtk_nand_hynix_rrtry(struct mtd_info *mtd, flashdev_info deviceinfo, u32 retryCount, bool defValue)
{
    if(defValue == FALSE)
    {
...
static void mtk_nand_hynix_16nm_rrtry(struct mtd_info *mtd, flashdev_info deviceinfo, u32 retryCount, bool defValue)
{
    if(defValue == FALSE)
    {
		if(g_hynix_retry_count == READ_RETRY_STEP)
...
int mtk_nand_exec_read_page(struct mtd_info *mtd, u32 u4RowAddr, u32 u4PageSize, u8 * pPageBuf, u8 * pFDMBuf)
{
    ...
    if (mtk_nand_ready_for_read(nand, u4RowAddr, 0, u4SecNum, true, buf))
    {
        ...
        if (g_bHwEcc)
        {
			...
			else
			{
				if(0 != (DRV_Reg32(NFI_STA_REG32) & STA_READ_EMPTY)) // if empty
				{
					if(retryCount != 0)
					{
						MSG(INIT, "read retry read empty page, return as uncorrectable\n");
				...
				for(i = 0; i < u4SecNum; i++)
				{
					...
					{	
						memcpy((rrtry_buffer+(gn_devinfo.sectorsize*i)), \
							(buf+(gn_devinfo.sectorsize*i)), gn_devinfo.sectorsize);
						memcpy((rrtry_buffer+mtd->writesize+(8*i)), (pFDMBuf+(8*i)), 8);
					}
				}
				bitMap |= tempBitMap;
			}
		...
		{
                        tempBitMap = 0;
			u32 feature = mtk_nand_rrtry_setting(gn_devinfo, gn_devinfo.feature_set.FeatureSet.rtype,gn_devinfo.feature_set.FeatureSet.readRetryStart,retryCount);
			...
			else
			{
				feature = gn_devinfo.feature_set.FeatureSet.readRetryDefault;
								// sandisk case 2/3/4
    		    if((gn_devinfo.feature_set.FeatureSet.rtype == RTYPE_SANDISK) && (g_sandisk_retry_case < 3))
    		    {
	...
	if(retryCount != 0)
	{
	    ...
	    if(bRet == ERR_RTN_SUCCESS)
	    {
			...
			mtd->ecc_stats.corrected++;
			if((gn_devinfo.feature_set.FeatureSet.rtype == RTYPE_HYNIX_16NM) || (gn_devinfo.feature_set.FeatureSet.rtype == RTYPE_HYNIX))
			{
				g_hynix_retry_count--;
			}
	    }
...
//	int retryCount = 0;
	...
	//MSG(INIT,"[WRITE] %d, %d, %d %d\n",mapped_block, block, page_in_block, page_per_block);
		...
		bRet = mtk_nand_exec_read_sector(mtd, page_in_block + mapped_block * page_per_block, coladdr, gn_devinfo.sectorsize*subpageno, buf, chip->oob_poi,subpageno);
	    if (bRet == ERR_RTN_SUCCESS)
    {
#ifdef DUMP_PEF
        do_gettimeofday(&etimer);
...
static int mtk_nand_probe(struct platform_device *pdev)
{
	...
	*NFI_CNRNB_REG16 = 0xF1;
	...
	{
	    nand_chip->ecc.layout->eccbytes = gn_devinfo.sparesize-OOB_AVAI_PER_SECTOR*(gn_devinfo.pagesize/sector_size);
	...
	{
		nand_chip->ecc.layout->eccbytes = 64;//gn_devinfo.sparesize-OOB_AVAI_PER_SECTOR*(gn_devinfo.pagesize/sector_size);
	    hw->nand_ecc_bytes = nand_chip->ecc.layout->eccbytes;
	    // Modify to fit device character    
	    nand_chip->ecc.size = hw->nand_ecc_size;    
	    nand_chip->ecc.bytes = hw->nand_ecc_bytes;  
	...
	nand_chip->ecc.layout->eccpos[i]=OOB_AVAI_PER_SECTOR*(gn_devinfo.pagesize/sector_size)+i;
    }
