...
static ssize_t  mtkdev_read(struct file *file, char __user *buffer, size_t len, loff_t *offset)
{
	ssize_t name_lens = 0;
	char buf[1024] = {'\0'};

	//len = sprintf(buf, "Boardinfo:\nCPU:\t%s\nLCM:\t%s\nTP:\t%s\nCAM:\t%s\nCAM2:\t%s\n\P_sensor:\t%s\n\G_sensor:\t%s\n\M_sensor:\t%s\n\n", &v_dev_name[CPU][0], &v_dev_name[LCM][0], &v_dev_name[TP][0], &v_dev_name[CAM][0], &v_dev_name[CAM2][0],\
	//				 &v_dev_name[P_sensor][0], &v_dev_name[G_sensor][0], &v_dev_name[M_sensor][0]);
	if(v_dev_name[M_sensor][0] != NULL)
	name_lens = sprintf(buf, "LCM:\t%s\nTP:\t%s\nM_CAM:\t%s\nS_CAM:\t%s\n\P_sensor:\t%s\n\G_sensor:\t%s\n\M_sensor:\t%s\n\n", &v_dev_name[LCM][0], &v_dev_name[TP][0], &v_dev_name[CAM][0], &v_dev_name[CAM2][0],\
					 &v_dev_name[P_sensor][0], &v_dev_name[G_sensor][0], &v_dev_name[M_sensor][0]);
	else
	name_lens = sprintf(buf, "LCM:\t%s\nTP:\t%s\nM_CAM:\t%s\nS_CAM:\t%s\n\P_sensor:\t%s\n\G_sensor:\t%s\n\n", &v_dev_name[LCM][0], &v_dev_name[TP][0], &v_dev_name[CAM][0], &v_dev_name[CAM2][0],\
					 &v_dev_name[P_sensor][0], &v_dev_name[G_sensor][0]);
	
	return simple_read_from_buffer(buffer, len, offset, buf,name_lens);
}

static  struct file_operations device_name_proc_fops = {
	.read = mtkdev_read,
	.write = NULL,
};
