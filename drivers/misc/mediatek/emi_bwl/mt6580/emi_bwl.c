...
int mtk_mem_bw_ctrl(int sce, int op)
{
    ...
    if (highest != cur_con_sce) {
        ...
        writel(emi_arbg_val[highest], EMI_ARBG);

        if(get_dram_type() == LPDDR3_1066) {
            writel(0x06171a2a, EMI_CONB);
            writel(0x07160616, EMI_CONC);
            mt_reg_sync_writel(0x2928172a, EMI_COND);
        } else if(get_dram_type() == LPDDR2_1066) {
            writel(0x06101a29, EMI_CONB);
            writel(0x07060607, EMI_CONC);
            mt_reg_sync_writel(0x0706061a, EMI_COND);
        }
        cur_con_sce = highest;
    }
