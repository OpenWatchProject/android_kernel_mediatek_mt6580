	...
	LINK_FOR_EACH_ENTRY(prBssDesc, prBSSDescList, rLinkEntry, BSS_DESC_T) {
		...
		if (!prCandidateBssDesc) {
			...
			if (fgIsFindBestRSSI) {
				 ...
				 */
				DBGLOG(SCN, TRACE, ("Candidate [" MACSTR "]: RCPI = %d, joinFailCnt=%d, Primary [" MACSTR"]: RCPI = %d, joinFailCnt=%d\n", 
				...
				if (prPrimaryBssDesc->ucJoinFailureCount > SCN_BSS_JOIN_FAIL_THRESOLD) {
					/* give a chance to do join if join fail before SCN_BSS_DECRASE_JOIN_FAIL_CNT_SEC seconds */
					if (CHECK_FOR_TIMEOUT(rCurrentTime, prBssDesc->rJoinFailTime,
						...
						MAC2STR(prBssDesc->aucBSSID), prBssDesc->ucJoinFailureCount, SCN_BSS_JOIN_FAIL_CNT_RESET_SEC));
					}
				}
