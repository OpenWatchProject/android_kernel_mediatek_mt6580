...
#if 1//fandian for wifi addr random  689C5EXXXXXX
				...
				while (1) {
					...
					if (u4Value & WMCSR_DL_OK) {
						...
						break;
						   ...
						   || fgIsBusAccessFailed == TRUE || (u4Value & WMCSR_DL_FAIL)) {
			 ...
			 IN UINT_32 u4DestAddr, IN UINT_32 u4ImgSecSize, IN PUINT_8 pucImgSecBuf)
{
	...
	HAL_PORT_WR(prAdapter,
		    MCR_FWDLDR,
		    prHifTxHeader->u2TxByteCount, (PUINT_8) prHifTxHeader, prAdapter->u4CoalescingBufCachedSize);
    ...
    int offset)
{
    ...
    if(IS_ERR(fd)) {
        ...
        DBGLOG(INIT, INFO, ("[MT6620][nvram_write] : succ to open!!\n"));
    }
    ...
    DBGLOG(INIT, INFO, ("nvram_write_wifiAddr retLen:%d\n",retLen));
    return retLen;
...
WLAN_STATUS wlanUpdateNetworkAddress(IN P_ADAPTER_T prAdapter)
{
    ...
    const UINT_8 aucSignalZeroMacAddr[] = {0x68, 0x9C, 0x5E, 0x00, 0x00, 0x00};
	...
	if (kalRetrieveNetworkAddress(prAdapter->prGlueInfo, &rMacAddr) == FALSE || IS_BMCAST_MAC_ADDR(rMacAddr)
            ...
            || EQUAL_MAC_ADDR(aucSignalZeroMacAddr, rMacAddr)) {
            #else		
	    || EQUAL_MAC_ADDR(aucZeroMacAddr, rMacAddr)) {
            #endif
		...
		if (prAdapter->fgIsEmbbededMacAddrValid == TRUE) {
...
#if 0 //fandian for wifi addr random  689C5EXXXXXX
			rMacAddr[0] = 0x00;
			rMacAddr[1] = 0x08;
			rMacAddr[2] = 0x22;
        	...
        	DBGLOG(INIT, TRACE, ("after write_wifi_file  = %d \n", retLen));
#endif
		}
	} else {
#if CFG_SHOW_MACADDR_SOURCE
...
#define NVRAM_5G_TX_BANDEDGE_OFDM40_OFFSET 12
	...
	/* printk("[frank] RSSI valid(%d) 2G(%d) 5G(%d)",
...
VOID StringTypeSetting(IN P_ADAPTER_T prAdapter, UINT_32 itemNum,  PUINT_32 pCurrentValue, BOOLEAN bSet)
{
	...
	if (bSet) {
		...
		printk("EasyDbg apply  %d CMD_STR_TEST_STR, val %s, %s\n", CMD_STR_TEST_STR, (PUCHAR)prAdapter->CfgSetting.TestStr, (PUCHAR)pCurrentValue);
	} else {
		prAdapter->QueryInfo.ptr = prAdapter->CfgSetting.TestStr;
		prAdapter->QueryInfo.Length = sizeof(prAdapter->CfgSetting.TestStr);
		prAdapter->QueryInfo.Type = ITEM_TYPE_STR;
		printk("EasyDbg apply  %d CMD_STR_TEST_STR, val %s\n", CMD_STR_TEST_STR, (PUCHAR)prAdapter->QueryInfo.ptr);
	}
	break;
	default:
		break;
...
VOID IntegerTypeSetting(IN P_ADAPTER_T prAdapter, UINT_32 itemNum, UINT_32 CurrentValue, BOOLEAN bSet)
{
	...
	case CMD_PNO_ENABLE:
		if (bSet) {
			...
			printk("EasyDbg apply  %d CMD_PNO_ENABLE, val 0x%x\n", CMD_PNO_ENABLE, CurrentValue);
		} else {
			prAdapter->QueryInfo.Value = prAdapter->CfgSetting.PnoEnable;
			prAdapter->QueryInfo.Length = sizeof(prAdapter->CfgSetting.PnoEnable);
			printk("EasyDbg Query , val 0x%x\n", prAdapter->CfgSetting.PnoEnable);
		}
		break;
	case CMD_PNO_SCAN_PERIOD:
		if (bSet) {
			prAdapter->CfgSetting.PnoScanPeriod = (UINT_32)CurrentValue;
			printk("EasyDbg apply  %d CMD_PNO_SCAN_PERIOD, val 0x%x\n", CMD_PNO_SCAN_PERIOD, CurrentValue);
		} else {
			...
			printk("EasyDbg Qyery , val 0x%x\n", prAdapter->CfgSetting.PnoScanPeriod);
		}
		break;
	case CMD_SCN_CHANNEL_PLAN:
		if (bSet) {
			...
			rlmDomainSendCmd(prAdapter, FALSE);
			printk("EasyDbg apply  %d CMD_SCN_CHANNEL_PLAN, val 0x%x\n", CMD_SCN_CHANNEL_PLAN, CurrentValue);
		} else {
			...
			printk("EasyDbg Qyery , val 0x%x\n", prAdapter->CfgSetting.ScnChannelPlan);
		}
		break;
	case CMD_SCN_DWELL_TIME:
		if (bSet) {
			prAdapter->CfgSetting.ScnDwellTime = (UINT_16)CurrentValue;
			printk("EasyDbg apply  %d CMD_SCN_DWELL_TIME, val 0x%x\n", CMD_SCN_DWELL_TIME, CurrentValue);
		} else {
			...
			printk("EasyDbg Qyery , val 0x%x\n", prAdapter->CfgSetting.ScnDwellTime);
		}
		break;
	case CMD_SCN_STOP_SCAN:
		if (bSet) {
			prAdapter->CfgSetting.ScnStopScan = (UINT_8)CurrentValue;
			printk("EasyDbg apply  %d CMD_SCN_STOP_SCAN, val 0x%x\n", CMD_SCN_STOP_SCAN, CurrentValue);
		} else {
			...
			printk("EasyDbg Qyery , val 0x%x\n", prAdapter->CfgSetting.ScnStopScan);
		}
		break;
	default:
		break;
...
VOID InitDefaultSetting(IN P_ADAPTER_T prAdapter)
{
	...
	printk("==> EasyDbg InitDefaultSetting total items %d\n", NIC_NUM_REG_PARAMS);
	...
	for (i = 0; i < NIC_NUM_STR_REG_PARAMS; i++) {
		...
		RecordCfgSetting(prAdapter, itemNum, itemType, g_DefaultStrCfgTable[i].DefString, TRUE);
	}
	printk("<== EasyDbg InitDefaultSetting\n");
}
...
VOID UpdateSettingFromExternal(IN P_ADAPTER_T prAdapter, P_CMD_HEADER_T prtGenBuff, BOOLEAN bSet)
{
	...
	printk("==> EasyDbg UpdateSetting total items %d,  Int:%d, Str:%d, bSet %d, type %d\n",
		itemSetNum,
		NIC_NUM_REG_PARAMS,
	...
	for (j = 0; j < itemSetNum; j++) {
		for (i = 0; i < NIC_NUM_REG_PARAMS; i++) {
			...
			printk("EasyDbg search Int %s, %s, length %d, default val 0x%x\n",
				prtCmdForMatBuf->itemString,
				g_DefaultCfgTable[i].String,
				prtCmdForMatBuf->itemValueLength,
				CurrentValue);
			...
			/*Integer case:*/
				...
				if (prtCmdForMatBuf->itemValueLength <= 4) {
					...
					/*4 byte value. */
						...
						printk("==> EasyDbg Unknown case !!!\n");

					printk("EasyDbg Find Int %s, %s, length %d, default val 0x%x, new value 0x%x\n",
						prtCmdForMatBuf->itemString,
						g_DefaultCfgTable[i].String,
						prtCmdForMatBuf->itemValueLength,
		...
		for (i = 0; i < NIC_NUM_STR_REG_PARAMS; i++) {
			UINT_32 itemNum = 0xffffffff;
			PUINT_8 pStrValue = g_DefaultStrCfgTable[i].DefString;
			itemNum = g_DefaultStrCfgTable[i].itemNum;
			...
			if ((strcmp(g_DefaultStrCfgTable[i].String, prtCmdForMatBuf->itemString) == 0)) {
				...
				printk("EasyDbg search Str %s, %s, length %d, default str %s, new str %s\n",
					prtCmdForMatBuf->itemString,
					g_DefaultStrCfgTable[i].String,
					prtCmdForMatBuf->itemValueLength,
	...
	printk("<== EasyDbg UpdateSettingFromExternal\n");
...
VOID wlanGetSetCfgFromExt (IN P_ADAPTER_T prAdapter, P_CMD_HEADER_T prtCmdHeader)
{
	...
	if (prtCmdHeader->cmdType == CMD_TYPE_SET) {
		...
		printk("EasyDbg pGetValue =  len %d,	Value %d\n", pGetValue->Length, pGetValue->Value);
		...
		if (pGetValue->Length <= 4) {
			...
			if (pGetValue->Type != ITEM_TYPE_STR) {
				...
				printk("EasyDbg  get Str %s", prtCmdHeader->buffer);
			}
		}
	}
}
...
WLAN_STATUS wlanCfgParseToFW(PCHAR *args, PCHAR args_size, UCHAR nargs, PCHAR buffer, UCHAR times)
{
	...
	for (i = 0; i <  cmd_v1.itemStringLength; i++)
		printk("%c", cmd_v1.itemString[i]);
	printk("\n");

	printk("cmd_v1.itemType = %d\n", cmd_v1.itemType);
	if (cmd_v1.itemType == ITEM_TYPE_DEC || cmd_v1.itemType == ITEM_TYPE_HEX) {
		...
		case ITEM_TYPE_HEX:
			...
			if (args_size[ED_VALUE_SITE] < 3 || ch != '0') {
				printk("Hex args must have prefix '0x'\n");
				return WLAN_STATUS_FAILURE;
			}
	...
	printk("Length = %d itemValue:", cmd_v1.itemValueLength);
	for (i = cmd_v1.itemValueLength - 1; i >= 0; i--)
		printk("%d,", cmd_v1.itemValue[i]);
	printk("\n");
...
WLAN_STATUS wlanCfgParse(IN P_ADAPTER_T prAdapter, PUINT_8 pucConfigBuf, UINT_32 u4ConfigBufLen, BOOLEAN isFwConfig)
{
	...
	cmdV1Header.cmdType = CMD_TYPE_SET;
	cmdV1Header.cmdVersion = CMD_VER_1;
	cmdV1Header.cmdBufferLen = 0;
	...
	for (;;) {
		...
		case STATE_EOF:
			...
			if (nargs == 2 && !isFwConfig)
				...
				WLAN_STATUS ret;
				ret = wlanCfgParseToFW(args, apcArgv_size, nargs, cmdV1Header.buffer, times);
		...
		case STATE_NEWLINE:
			if (nargs == 2 && !isFwConfig) {
				...
				if (times == MAX_CMD_ITEM_MAX) {
					/* Send to FW */
					wlanGetSetCfgFromExt(prAdapter, &cmdV1Header);
					
					cmdV1Header.itemNum = times;
					...
					if (rStatus == WLAN_STATUS_FAILURE)
						printk("kalIoctl wifiSefCFG fail 0x%x\n", rStatus);
					printk("kalIoctl wifiSefCFG num:%d X\n", times);
					memset(cmdV1Header.buffer, 0, MAX_CMD_BUFFER_LENGTH);
					cmdV1Header.cmdBufferLen = 0;
					times = 0;
				}
			}
		...
		case STATE_TEXT:
			if (nargs < WLAN_CFG_ARGV_MAX) {
				...
				state.textsize = 0;
				printk(" nargs= %d STATE_TEXT = %s, SIZE = %d\n",
					nargs-1, args[nargs-1], apcArgv_size[nargs-1]);
			}
			break;
		}
	}
