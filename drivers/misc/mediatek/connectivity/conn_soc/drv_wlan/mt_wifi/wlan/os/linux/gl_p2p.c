...
VOID p2pUpdateChannelTableByDomain(P_GLUE_INFO_T prGlueInfo)
{
	...
	for (i = 0; i < ucNumOfChannel; i++) {
		...
		case BAND_2G4:
			p2pEnableChannel(&uc2gChIdx, aucChannelList[i].ucChannelNum, mtk_2ghz_channels,
					 ARRAY_SIZE(mtk_2ghz_channels));
			break;

		case BAND_5G:
			p2pEnableChannel(&uc5gChIdx, aucChannelList[i].ucChannelNum, mtk_5ghz_a_channels,
					 ARRAY_SIZE(mtk_5ghz_a_channels));
			break;
