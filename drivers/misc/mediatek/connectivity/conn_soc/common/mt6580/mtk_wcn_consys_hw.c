...
#if CONSYS_PMIC_CTRL_ENABLE
		if(reg_V33_BT){
			if(regulator_enable(reg_V33_BT)){
				WMT_PLAT_INFO_FUNC("WMT do BT PMIC on fail!\n");
				return -1;
			}
		}
#endif
...
#if CONSYS_PMIC_CTRL_ENABLE
		if(reg_V33_WIFI){
			if(regulator_enable(reg_V33_WIFI)){
				WMT_PLAT_INFO_FUNC("WMT do WIFI PMIC on fail!\n");
				return -1;
			}
		}
#endif
