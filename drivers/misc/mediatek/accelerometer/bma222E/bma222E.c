...
static struct acc_hw cust_acc_hw = {
    .i2c_num = 2,
	#if defined(CONFIG_Z68_PROJECT)||defined(CONFIG_Z69_PROJECT)||defined(CONFIG_G501_PROJECT)
	.direction = 1,
	#elif defined(CONFIG_Z62_PROJECT)
	.direction = 4,//4
	#else
	.direction = 4,
	#endif
	...
	int act[BMA_AXES_NUM], int raw[BMA_AXES_NUM])
{
	...
	*/
	...
	raw[BMA_AXIS_X] =
		obj->offset[BMA_AXIS_X]*mul*GRAVITY_EARTH_1000/obj->sensitivity + obj->cali_sw[BMA_AXIS_X];
	raw[BMA_AXIS_Y] =
		obj->offset[BMA_AXIS_Y]*mul*GRAVITY_EARTH_1000/obj->sensitivity + obj->cali_sw[BMA_AXIS_Y];
	raw[BMA_AXIS_Z] =
		obj->offset[BMA_AXIS_Z]*mul*GRAVITY_EARTH_1000/obj->sensitivity + obj->cali_sw[BMA_AXIS_Z];
	...
	int dat[BMA_AXES_NUM])
{
	...
	obj->offset[BMA_AXIS_X] = (s8)(obj->cvt.sign[BMA_AXIS_X]*
		(cali[obj->cvt.map[BMA_AXIS_X]])*obj->sensitivity/GRAVITY_EARTH_1000/(divisor));
	obj->offset[BMA_AXIS_Y] = (s8)(obj->cvt.sign[BMA_AXIS_Y]*
		(cali[obj->cvt.map[BMA_AXIS_Y]])*obj->sensitivity/GRAVITY_EARTH_1000/(divisor));
	obj->offset[BMA_AXIS_Z] = (s8)(obj->cvt.sign[BMA_AXIS_Z]*
		(cali[obj->cvt.map[BMA_AXIS_Z]])*obj->sensitivity/GRAVITY_EARTH_1000/(divisor));
...
static int bma_get_chip_type(struct i2c_client *client)
{
	...
	printk("bma_get_chip_type i2c OK\n");
	switch (chip_id) {
	case BMA220_CHIP_ID:
...
static int bma_init_client(struct i2c_client *client, int reset_cali)
{
	...
	printk("########### bma_init_client get chip type OK\n");
		...
		char *buf, int bufsize)
{
	...
	#if 1
		databuf[BMA_AXIS_X] = databuf[BMA_AXIS_X] *
			GRAVITY_EARTH_1000 / obj->sensitivity;
		databuf[BMA_AXIS_Y] = databuf[BMA_AXIS_Y] *
			GRAVITY_EARTH_1000 / obj->sensitivity;
		databuf[BMA_AXIS_Z] = databuf[BMA_AXIS_Z] *
			GRAVITY_EARTH_1000 / obj->sensitivity;
		#endif
		/* compensate data */
...
//	v_set_dev_name(6, "bma2xx");
	
	GSE_LOG("%s: OK\n", __func__);
//	extern void v_set_dev_name(int id, char *name);
//	v_set_dev_name(6, "bma222E");
	return 0;
