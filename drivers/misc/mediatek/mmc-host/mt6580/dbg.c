...
static int smp_test_on_one_host(int thread_num, int host_id, int count, int multi_address)
{
	...
	for (i = 0; i < thread_num; i++) {
		...
		case MMC_TYPE_SDIO:
			if (i == 0) {
				pr_err(" MSDC[%d], SDIO:\n", host_id);
				pr_err("    please manually trigger wifi application");
				pr_err(" instead of write/read something on SDIO card\n");
			}
			ret = -1;
			goto out;
...
static int smp_test_on_all_host(int thread_num, int count, int multi_address)
{
	...
	for (i = 0; i < HOST_MAX_NUM; i++) {
		...
		if (host_ctl->mmc->card->type == MMC_TYPE_SDIO) {
			pr_err(" MSDC[%d], SDIO, please manually trigger wifi ", i);
			pr_err("application instead of write/read something on SDIO card\n");
			continue;
		}
...
static int msdc_debug_proc_show(struct seq_file *m, void *v)
{
	...
	for (i = 0; i < HOST_MAX_NUM; i++)
		seq_printf(m, "-> MSDC[%d] Zone: 0x%.8x\n", i, sd_debug_zone[i]);
	...
	for (i = 0; i < HOST_MAX_NUM; i++)
		seq_printf(m, "-> Clokc SRC selection Host[%d]<%d>\n", i, msdc_clock_src[i]);
				     ...
				     loff_t *data)
{
	...
	int spd_mode = CAPS_SPEED_NULL;
	int drv_type = CAPS_DRIVE_NULL;
	int current_limit = CAPS_CURRENT_NULL;
	int pw_cr = CAPS_POWER_NULL;
	...
	if (cmd == SD_TOOL_ZONE) {
		...
		if (id >= 0 && id < HOST_MAX_NUM) {
			if (p1 == 0) {
				if (p3 >= 0 && p3 < CLK_SRC_MAX_NUM) {
					...
					pr_err("[****SD_Debug****]to enable the above settings,");
					pr_err(" please suspend and resume the phone again\n");
				} else {
					pr_err
		...
		if (id >= HOST_MAX_NUM || id < 0)
			...
			if ((p2 < 0) || (p2 > 2))
				...
				pr_err("[****SD_Debug****]Some rd/td value was not");
				pr_err(" right(rd mask:(0x3F << 4),td mask:(0xF << 0))\n");
			} else {
#ifndef FPGA_PLATFORM
				if (p2 == 0) {	/* set rd:0 */
...
static int msdc_debug_proc_read_FT_show(struct seq_file *m, void *data)
{
	...
	sdr_get_field(MSDC_PATCH_BIT0, MSDC_PB0_RD_DAT_SEL, data_edge);
/*
	...
	seq_puts(m, "msdc only using internal clock\n");
...
/*
		...
		sdr_set_field(MSDC_IOCON, MSDC_IOCON_RSPL, i_edge);
		sdr_set_field(MSDC_PATCH_BIT0, MSDC_PB0_RD_DAT_SEL, i_edge);
				       ...
				       loff_t *data)
{
	...
	sscanf_num = sscanf(cmd_buf, "%d %d %d", &sdio_vio18_flag, &sdio_vcore1_flag, &sdio_vcore2_flag);
	if (sscanf_num < 1)
		return count;
