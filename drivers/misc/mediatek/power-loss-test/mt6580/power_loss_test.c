...
int pwr_loss_open(struct inode *inode, struct file *file)
{
	...
	pr_err("%s Power Loss Test: Open operation !\n", TAG);
#endif
	return 0;
}
...
{
#ifdef PWR_LOSS_DEBUG
	pr_err("%s Power Loss Test: Release operation !\n", TAG);
#endif
	return 0;
}
...
long pwr_loss_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
...
#ifdef PWR_LOSS_DEBUG
	pr_err("%s Power Loss Test: IOCTL operation ! CMD=%d\n", TAG, cmd);
#endif
	...
	case PRINT_REBOOT_TIMES:
		...
		if (ret != 0) {
			pr_err(
			       "%s Power Loss Test: IOCTL->PRINT_REBOOT_TIMES %d Bytes can't be copied\n",
			       TAG, ret);
		}
		pr_err(
		       "%s Power Loss Test: -----------System Reboot Successfully Times= %d---------------!\n",
		       TAG, ((int *)l_buf)[0]);
		break;

	case PRINT_DATA_COMPARE_ERR:
		pr_err(
		       "%s Power Loss Test: -----------Data Compare Error---------------!\n", TAG);
		break;

	case PRINT_FILE_OPERATION_ERR:
		pr_err(
		       "%s Power Loss Test: -----------File Operation Error---------------!\n",
		       TAG);
		ret = copy_from_user(l_buf, (int *)arg, (sizeof(l_buf[0]) * (sizeof(l_buf))));
		if (ret != 0) {
			pr_err(
			       "%s Power Loss Test: IOCTL->PRINT_FILE_OPERATION_ERR %d Bytes can't be copied\n",
			       TAG, ret);
		}
	...
	case PRINT_GENERAL_INFO:
		...
		if (ret != 0) {
			pr_err(
			       "%s Power Loss Test: IOCTL->PRINT_GENERAL_INFO %d Bytes can't be copied\n",
			       TAG, ret);
		}
	...
	case PRINT_RAW_RW_INFO:
		...
		if (ret != 0) {
			pr_err(
			       "%s Power Loss Test: IOCTL->PRINT_RAW_RW_INFO %d Bytes can't be copied\n",
			       TAG, ret);
		}
	...
	default:
		pr_err("%s Power Loss Test: cmd code Error!\n", TAG);
		break;
	}
...
static int power_loss_debug_write(struct file *file, const char *buf, size_t count, loff_t *data)
{
	...
	if (1 == sscanf(cmd_buf, "%x", &wdt_reboot_support)) {
		if (wdt_reboot_support < 0) {
			pr_err(
			       "%s [%s] : command format is error, please help to check!!!\n", TAG,
			       __func__);
			return -1;
		} else {
			down_write(&power_loss_info.rwsem);
			...
			pr_err("%s [****PWR_LOSS_DEBUG****]\n", TAG);
			pr_err("%s WDT REBOOT\t", TAG);
...
static int power_loss_debug_init(void)
{
	...
	if (prEntry) {
		pr_err("[%s]: successfully create /proc/power_loss_debug\n", __func__);
	} else {
		pr_err("[%s]: failed to create /proc/power_loss_debug\n", __func__);
		return -1;
	}
...
static int __init power_loss_init(void)
{
	...
	pr_err("%s Power Loss Test Module Init\n", TAG);

	err =
	    alloc_chrdev_region(&sg_pwr_loss_devno, PWR_LOSS_FIRST_MINOR, PWR_LOSS_MAX_MINOR_COUNT,
				PWR_LOSS_DEVNAME);
	if (err != 0) {
...
#ifdef PWR_LOSS_DEBUG
	pr_err("%s Power Loss Test: MAJOR =%d, MINOR=%d\n", TAG,
	...
	sg_pwr_loss_dev_file =
	    device_create(sg_pwr_loss_dev_class, NULL, sg_pwr_loss_devno, NULL, PWR_LOSS_DEVNAME);
...
{
#ifdef PWR_LOSS_DEBUG
	pr_err("%s Power Loss Test: Module Exit\n", TAG);
#endif
