...
static unsigned int md1_dbg_mode_ptr; 
...
int get_dts_settings(int md_id) {
    ...
    // of_property_read_u32(node, "cldma,major", &dev_cfg->major);
    // of_property_read_u32(node, "cldma,minor_base", &dev_cfg->minor_base);
    // of_property_read_u32(node, "cldma,capability", &dev_cfg->capability);

    // cldma_ap_ao_base = of_iomap(node, 0);
    // cldma_md_ao_base = of_iomap(node, 1);
    // cldma_irq_id = irq_of_parse_and_map(node, 0);
    // ap_ccif_irq_id = irq_of_parse_and_map(node, 1);
    node = of_find_compatible_node(NULL, NULL, "mediatek,INFRACFG_AO");
    if(!node){
        CCCI_ERR_INF(md_id, "plt", "md%d INFRACFG_AO is not set in DT!\n", (md_id+1));
        // return -1;
    } else {
    ...
    if(!node){
        CCCI_ERR_INF(md_id, "plt", "md%d MCUSYS_CFGREG_BASE is not set in DT!\n", (md_id+1));
        // return -1;
    } else {
    ...
    if(!node){
        CCCI_ERR_INF(md_id, "plt", "md%d smem size is not set in device tree,need to check\n", (md_id+1));
        return -1;
    }
...
static void enable_mem_access_protection(int md_id)
{
    ...
    shr_mem_phy_end = ((shr_mem_phy_end + 0xFFFF) & (~0xFFFF)) - 1;  //end addr: EMI MPU ignore the lowest 16bit
    rom_mem_phy_start = md_layout->md_region_phy;
    rom_mem_phy_end   = ((md_layout->md_region_phy + img_info[MD_INDEX].size + 0xFFFF) & (~0xFFFF)) - 1;

    rw_mem_phy_start  = (md_layout->md_region_phy + img_info[MD_INDEX].size + 0xFFFF) & (~0xFFFF);  // 64KB align
...
void md_dsp_wdt_irq_en(int md_id)
{
    ...
    {
        case MD_SYS1:
            irq_id = md_wdt_irq_id[md_id];  // MD_WDT_IRQ_ID;
