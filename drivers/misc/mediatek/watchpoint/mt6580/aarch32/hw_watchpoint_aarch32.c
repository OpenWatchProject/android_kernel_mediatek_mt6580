...
static int __init hw_watchpoint_init(void)
{
	...
	node = of_find_compatible_node(NULL, NULL, "mediatek,DBG_DEBUG");
	if (!node) {
	...
	if (!MT_HW_DBG_BASE) {
		pr_err("[WP] kzalloc failed for MT_HW_DBG_BASE.\n");
		return -ENOMEM;
	}

	wp_events = kzalloc(sizeof(struct wp_event) * nr_dbg, GFP_KERNEL);
	if (!wp_events) {
		pr_err("[WP] kzalloc failed for wp_events\n");
		return -ENOMEM;
	}
