 ...
 */
static int polling_trip_temp1 = 35000;
...
//static int CPU_TS_MCU1_T=0;
static int CPU_TS_MCU2_T=0;
//static int CPU_TS_MCU3_T=0;

//static int CPU_TS_MCU1_R=0;
...
#define ADDRESS_INDEX_0	 42  //0xF020619C
#define ADDRESS_INDEX_1	 43  //0xF02061A0
#define ADDRESS_INDEX_2	 44  //0xF02061A4
#endif
...
//#define TEMPADC_MCU4    (TSCON1_bit_4_5_00|TSCON1_bit_0_2_011)
//#define TEMPADC_ABB     (TSCON1_bit_4_5_10|TSCON1_bit_0_2_000)
#endif
...
static int bank_ts[THERMAL_BANK_NUM][ENUM_MAX];
static int bank_ts_r[THERMAL_BANK_NUM][ENUM_MAX];
...
static int thermal_ADCValueOfMcu(ts_e type)
{
     switch (type) {
          case MCU1 :
               return TEMPADC_MCU1;
...
#define TS_MS_TO_NS(x) (x * 1000 * 1000)
static struct hrtimer ts_tempinfo_hrtimer;
static int g_hr_timer_polling_delay = TS_MS_TO_NS(1000);//default=50 ms
...
static void thermal_config_Bank0_TS(void)
{
	...
	THERMAL_WRAP_WR32(TS_CONFIGURE_P,TEMPPNPMUXADDR);  	// AHB address for pnp sensor mux selection
...
{
	//tscpu_dprintk( "thermal_enable_all_periodoc_sensing_point bank_num=%d\n",bank_num);
...
static void read_all_bank_temperature(void)
{
    ...
    for (i=0; i<TS_LEN_ARRAY(g_bank); i++) {
        ...
        for (j=0; j<g_bank[i].ts_number; j++) {
            thermal_read_bank_temp(i,g_bank[i].ts[j].type,j);
    }
    }
...
static int max_temperature(void)
{
    int i = 0;
    int max = 0;
    int max_in_bank = 0;
    ...
    for (i=0; i<TS_LEN_ARRAY(g_bank); i++) {
        ...
        if (max_in_bank > max) max = max_in_bank;
    }
							...
							unsigned long *t)
{
	...
	//tscpu_printk("tscpu_get_temp CPU TS1=%d\n",get_immediate_ts1_wrap());
	//tscpu_printk("tscpu_get_temp CPU TS2=%d\n",get_immediate_ts2_wrap());
...
static int decide_ttj(void)
{
    ...
    if (ctm_on == 1)
    {
        ...
        TARGET_TJ = MIN(MAX_TARGET_TJ, MAX(STEADY_TARGET_TJ, (COEF_AE - COEF_BE*curr_tpcb/1000)));
...
static void tscpu_clear_all_temp(void)
{
    ...
    for (i=0; i<TS_LEN_ARRAY(g_bank); i++) {
         for (j=0; j<g_bank[i].ts_number; j++) {
            bank_ts[i][g_bank[i].ts[j].type] = CLEAR_TEMP;
         }
    }
}
...
static int tscpu_thermal_suspend(struct platform_device *dev, pm_message_t state)
{
    ...
    if(talking_flag==false)
    {
		...
		THERMAL_WRAP_WR32(DRV_Reg32(TS_CONFIGURE) | TS_TURN_OFF, TS_CONFIGURE); // turn off the sensor buffer to save power
    }
...
void tscpu_pause_tc(void)
{
	...
	//if(talking_flag==false)
	{
		...
		/*TSCON1[5:4]=2'b11, Buffer off*/
		//THERMAL_WRAP_WR32(DRV_Reg32(TS_CON1_TM) | 0x00000030, TS_CON1_TM); // turn off the sensor buffer to save power
	}
...
//if((DRV_Reg32(TS_CON1_TM)& 0x00000030) == 0x11)
    ...
    if (thz_dev != NULL && interval != 0)
	    mod_delayed_work(system_freezable_wq, &(thz_dev->poll_queue), round_jiffies(msecs_to_jiffies(1000))); // 1000ms
}
...
static void init_thermal(void)
{
	...
	*/
    ...
    //BUG_ON(clock_is_on(MT_CG_PERI_AUXADC) == 0x0); [Rainier]Fix ME
