...
int wifi_in_soc_throttle_from_cpu(int state);
...
static int resource_allocator_policy = 4; //0: wifi last (default) 1: wifi first 2: no throttle wifi 3: independent 4: not share target tj
static int min_wifi_tput_ratio = 40;
static int max_wifi_tput_ratio = 200;
static int min_wifi_tput = 1000;
static int tt_wifi_high = 50; // inital value: assume 1 degreeC for temp. <=> 1 unit for wifi_tput_ratio(0~100)
static int tt_wifi_low = 50;
static int tp_wifi_rise = 10000;
static int tp_wifi_fall = 10000;
static int triggered = 0; //wt2
...
static unsigned long set_adaptive_wifi_tput_limit(unsigned int cur_tput, unsigned int limit_ratio)
{
	...
	wmt_tm_dprintk("%s: Curr Tput= %lu ,ratio= %u, limit_tput= %d",__func__,tx_throughput,limit_ratio,limit_tput);
...
static int adaptive_tput_ratio(long prev_temp, long curr_temp)
{
    ...
    if (cl_dev_adp_cpu_state_active == 1) {
        ...
        if(((curr_temp > WIFI_TARGET_TJ_HIGH) && (curr_temp > prev_temp)) ||
            ((curr_temp <= WIFI_TARGET_TJ_HIGH) && (curr_temp >= WIFI_TARGET_TJ_LOW) && (curr_temp >= (prev_temp + (tj_stable_range * 2))))){
        ...
        wifi_tput_ratio = (wifi_tput_ratio > min_wifi_tput_ratio ) ? wifi_tput_ratio : min_wifi_tput_ratio;
        wifi_tput_ratio = (wifi_tput_ratio < max_wifi_tput_ratio) ? wifi_tput_ratio : max_wifi_tput_ratio;
        wmt_tm_dprintk("%s Wifi TJ %d Tp %ld, Tc %ld, wifi_tput_ratio %d tput %lu\n", __func__, wifi_target_tj, prev_temp, curr_temp, wifi_tput_ratio,tx_throughput);
...
static void heterogeneous_resource_allocator(int temp)
{
    ...
    wmt_tm_dprintk("%s: temp= %d policy= %d target_tj= %d target_offset= %d\n", __func__, temp,resource_allocator_policy,wifi_target_tj,wifi_target_offset);
    ...
    switch(resource_allocator_policy){
        case 0://wifi last
            if(is_cpu_power_unlimit()){
                ...
                cl_dev_adp_cpu_state_active = (temp >= (wifi_target_tj - wifi_target_offset)) ? 1 : 0;
                adaptive_tput_ratio(g_prev_temp, temp);
            } else {
                if(!is_wifi_tput_min()){
                    set_adaptive_wifi_tput_limit(tx_throughput, min_wifi_tput_ratio);
                }
                triggered = 1; //wt2
            }
            break;
        ...
        case 3://independent
		    wifi_target_offset = wmt_wifi_target_offset;
            if(is_cpu_power_unlimit())
				wifi_target_tj = wmt_wifi_target_tj;// min(wmt_wifi_target_tj, get_cpu_target_tj() - cpu_wifi_target_tj_offset);
            else
				wifi_target_tj = min(wmt_wifi_target_tj, get_cpu_target_tj() - cpu_wifi_target_tj_offset);
...
static int wmt_cal_stats(unsigned long data)
{
	...
	if (pre_time != 0 && cur_time.tv_sec > pre_time) {
		...
		if (tx_bytes > stats_info->pre_tx_bytes) {
			...
			wmt_tm_dprintk("[%s] cur_time=%lu, cur_data=%lu, tx_throughput=%luK bit/s(%luK Byte/s)\n", __func__, cur_time.tv_sec, tx_bytes, tx_throughput, tx_throughput>>3 );
                            ...
                            unsigned long *pv)
{
	...
	if(sensor_select < 0 || sensor_select >= NR_TS_SENSORS){
		aee_kernel_warning_api(__FILE__, __LINE__, DB_OPT_DEFAULT, "wmt_thz_get_temp ","sensor_select: %d\n", sensor_select);
		sensor_select = 0;
	}
...
#else
    return single_open(file, wmt_wifi_tx_thro_limit_read, PDE(inode)->data);
#endif
}
...
ssize_t wmt_wifi_in_soc_write(struct file *filp, const char __user *buf, size_t len, loff_t *data)
{
	int ret = 0;
	char tmp[MAX_LEN] = {0};
...
#else
    return single_open(file, wmt_wifi_in_soc_read, PDE(inode)->data);
#endif
}
...
static int wmt_tm_proc_register(void)
{
    ...
    if (!wmt_thro_proc_dir) {
        ...
        entry = proc_create("tx_thro", S_IRUGO | S_IWUSR, wmt_thro_proc_dir, &_tx_thro_fops);
        entry = proc_create("tx_thro_limit", S_IRUGO | S_IWUSR, wmt_thro_proc_dir, &_tx_thro_limit_fops);
    }
    ...
    if (!wmt_tm_proc_dir) {
        ...
        entry = proc_create("wifi_in_soc", S_IRUSR | S_IWUSR | S_IRGRP | S_IWGRP, wmt_tm_proc_dir,
                &_wifi_in_soc_fops);
        if (entry) {
#if LINUX_VERSION_CODE >= KERNEL_VERSION(3, 10, 0)
            proc_set_user(entry, 0, 1000);
