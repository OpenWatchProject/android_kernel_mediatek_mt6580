...
static int mtkfb_fbinfo_init(struct fb_info *info)
{
    ...
    if (r != 0)
        printk("unable to allocate color map memory\n");
    ...
    if (r != 0)
        printk("failed to mtkfb_check_var\n");

    info->var = var;

    r = mtkfb_set_par(info);
    if (r != 0)
        printk("failed to mtkfb_set_par\n");
    return r;
}
...
int _parse_tag_videolfb(void)
{
	...
	if(of_scan_flat_dt(fb_early_init_dt_get_chosen, NULL) > 0)
	{
		...
		else
		{
            ...
            if(!prop){
                printk("[DT][videolfb] fail to parse islcmfound\n");
                return -1;
            }
            ...
            if(!prop){
                printk("[DT][videolfb] fail to parse fps\n");
                return -1;
            }
            ...
            if(!prop){
                printk("[DT][videolfb] fail to parse vramSize\n");
                return -1;
            }
            ...
            if(!prop){
                printk("[DT][videolfb] fail to parse lcmname\n");
                return -1;
            }
...
int mtkfb_allocate_framebuffer(phys_addr_t pa_start, phys_addr_t pa_end, unsigned int* va, unsigned int* mva)
{
	int ret = 0;
...
static int mtkfb_probe(struct device *dev)
{
    ...
    if (!fbi) 
    {
        printk("unable to allocate memory for device info\n");
        r = -ENOMEM;
        goto cleanup;
    }
    ...
    if (r)
    {
    	printk("mtkfb_fbinfo_init fail, r = %d\n", r);
        goto cleanup;
    }
    init_state++;   // 4
	mtkfb_fbi = fbi;
...
int __init mtkfb_init(void)
{
	...
	{
		printk("failed to register mtkfb driver\n");
		r = -ENODEV;
	}
    return r;
}
