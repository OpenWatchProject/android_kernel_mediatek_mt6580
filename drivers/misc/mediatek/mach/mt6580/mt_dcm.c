...
int dcm_armcore_pll_clkdiv(int pll, int div)
{
	...
	//disable mainpll cg 
		if( 3 == pll)
		{
			reg_write(INFRA_RSVD1, 
				  aor(reg_read(INFRA_RSVD1), ~(1<<1), (0<<1))); 
			reg_write(MPLL_FREDIV_EN, 
				  aor(reg_read(MPLL_FREDIV_EN), ~(1<<0), (1<<0)));  
		}
...
int dcm_armcore_pll_gating(ENUM_ARMCORE_PLL_GATING mode)
{
	...
	{
		reg_write(INFRA_RSVD1, 
				  aor(reg_read(INFRA_RSVD1), ~(1<<2), (0<<2)));  //disable armpll ck gating

	}
	else if (mode == ARMCORE_PLL_GATING_ON)
	{
		reg_write(INFRA_RSVD1, 
				  aor(reg_read(INFRA_RSVD1), ~(1<<2), (1<<2)));  //enable armpll ck gating	
	}
...
int dcm_armcore(ENUM_ARMCORE_DCM mode)
{
        ...
        if (mode == ARMCORE_DCM_OFF) {
				...
				reg_write(CLK_GATING_CTRL0, 
										  aor(reg_read(CLK_GATING_CTRL0), ~ARM_EMICLK_WFI_GAT_DIS_MASK, ARM_EMICLK_WFI_GAT_DIS_OFF)); 
        ...
        if (mode == ARMCORE_DCM_MODE2) {
				...
				reg_write(CLK_GATING_CTRL0, 
						  aor(reg_read(CLK_GATING_CTRL0), ~ARM_EMICLK_WFI_GAT_DIS_MASK, ARM_EMICLK_WFI_GAT_DIS_ON)); 		
        }
        ...
        if (on == MCUSYS_DCM_OFF) {
				...
				//Whole  BUS DCM
                 MCUSYS_SMC_WRITE(MCUCFG_MISC_CONFIG, 
                                 aor(reg_read(MCUCFG_MISC_CONFIG), ~MCUCFG_L2_BUS_DCM_MASK, MCUCFG_L2_BUS_DCM_OFF));
				...
				MCUSYS_SMC_WRITE(MCU_BIU_CON, 
								aor(reg_read(MCU_BIU_CON), ~MCU_BIU_CON_DCM_EN_MASK, MCU_BIU_CON_DCM_EN_OFF));
                ...
                MCUSYS_SMC_WRITE(MCUCFG_MISC_CONFIG, 
                                 aor(reg_read(MCUCFG_MISC_CONFIG), ~MCUCFG_L2_BUS_DCM_MASK, MCUCFG_L2_BUS_DCM_ON)); 
				MCUSYS_SMC_WRITE(MCU_BIU_CON, 
								aor(reg_read(MCU_BIU_CON), ~MCU_BIU_CON_DCM_EN_MASK, MCU_BIU_CON_DCM_EN_ON));
        }
...
int dcm_pmic(ENUM_PMIC_DCM on)
{
        if (on) {
                reg_write(INFRABUS_DCMCTL1, 
                          aor(reg_read(INFRABUS_DCMCTL1), ~(PMIC_SPICLKDCM_EN_MASK|PMIC_BCLKDCM_EN_MASK), PMIC_DCM_EN_ON));
        }
        else {
				reg_write(INFRABUS_DCMCTL1, 
					   aor(reg_read(INFRABUS_DCMCTL1), ~(PMIC_SPICLKDCM_EN_MASK|PMIC_BCLKDCM_EN_MASK), PMIC_DCM_EN_OFF));
        }                
...
int dcm_emi_slow_down_ratio(ENUM_EMI_DCM_SLOW_DOWN_RATIO ratio)
{
    ...
    ratio &= 0x3;
    value = (ratio<<0);
    reg_write(EMI_CONH, aor(reg_read(EMI_CONH), 
                                              ~EMIDCM_RATIO_MASK,
                                              value));	
	return 0;
}
...
int dcm_emi_dbc(int on, int cnt)
{
        ...
        reg_write(TOPEMI_DCMCTL, 
                  aor(reg_read(TOPEMI_DCMCTL), 
                      ~(EMIDCM_DBC_CNT_MASK|EMIDCM_DBC_EN_MASK),
                      value));
...
int dcm_emi(ENUM_EMI_DCM on)
{
        if (on) {
            ...
            reg_write(TOPEMI_DCMCTL, aor(reg_read(TOPEMI_DCMCTL), 
                                        ~(EMIDCM_EN_MASK), 
                                        EMIDCM_EN_ON|EMIDCM_APB_SEL_DCM));
        }
        else {
			dcm_emi_dbc(0,0);			
