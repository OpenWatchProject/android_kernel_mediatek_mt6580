...
int mt_disable_clock(enum cg_clk_id id, const char *name)
{
...
#if 0   // XXX: debug for MT_CG_APDMA_SW_CG
    {
        ...
        if (MT_CG_APDMA_SW_CG == id)
        {
            ...
            if (!clk_cnt_rec[i].cnt) pr_err(KERN_EMERG "Assert @ %s\n", clk_cnt_rec[i].name);
...
static int mfg_non_gf_clkmux_sel_op(struct clkmux *mux, cg_clk_id clksrc)
{
	int err = 0;
	int i, clk, gfclk;
