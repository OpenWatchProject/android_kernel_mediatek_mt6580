...
static unsigned long get_generic_timer_rate(void)
{
	return generic_timer_rate;
}

u32 arch_timer_get_rate(void)
{
	return ((u32)generic_timer_rate);
}
...
static int __cpuinit generic_timer_setup(struct clock_event_device *clk)
{
    ...
    *this_cpu_clk = clk;
    ...
    clockevents_config_and_register(clk, (unsigned int)generic_timer_rate,
                    0xf, 0x7fffffff);
...
static int __init generic_timer_register(struct device_node *np)
{
    int err, i;
    unsigned int freq;
		int arch_timer_ppi[MAX_TIMER_PPI];
    
	  if (get_generic_timer_rate()) {
		...
		if (!of_property_read_u32(np, "clock-frequency", &freq))
			generic_timer_rate = (unsigned long)freq;
  	
#ifdef CONFIG_MTK_FPGA
			generic_timer_rate = (unsigned long)(freq / 13 * 6);		//13M would be 6M on FPGA
#endif 	
    ...
    timer_ppi = arch_timer_ppi[PHYS_NONSECURE_PPI];	//GIC_PPI_NS_PRIVATE_TIMER;				
#else    
    timer_ppi = arch_timer_ppi[PHYS_SECURE_PPI];		//GIC_PPI_PRIVATE_TIMER;
#endif
...
CLOCKSOURCE_OF_DECLARE(armv7_arch_timer, "arm,armv7-timer", generic_timer_register);
////////////////////////////////////END//////////////////////////////////////
