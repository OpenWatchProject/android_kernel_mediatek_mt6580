...
//#define CONFIG_CPU_DVFS_RAMP_DOWN 1
//#define CONFIG_CPU_DVFS_TURBO_MODE 1
...
//wait for ext_buck
#define VOLT_TO_EXTBUCK_VAL(volt) (((((volt) - 800) + 9) / 10) & 0x7F) //no use, reg_val = ((((val*10)-300000)/1000)+9)/10;
...
#define MAX_RETRY_COUNT (100)
	...
	spm_write(SPM_POWERON_CONFIG_SET, (SPM_PROJECT_CODE << 16) | (1U << 0)); //PROJECT CODE should be equal to 0xB16
...
static struct mt_cpu_freq_info opp_tbl_e1_0[] = {
	...
	OP(CPU_DVFS_FREQ9,  102500),
	OP(CPU_DVFS_FREQ10, 102500),
	OP(CPU_DVFS_FREQ11, 102500),
};
...
#else
	con1 = cpufreq_read(ARMPLL_CON1);//[Rainier] Check ARMPLL reg
#if 1 //[Rainier] CLKDIV?
    ckdiv1 = cpufreq_read(TOP_CKDIV1);
    ckdiv1 = _GET_BITS_VAL_(4 : 0, ckdiv1);
#endif
...
static unsigned int get_cur_volt_pmic_wrap(struct mt_cpu_dvfs *p)
{
	...
	//cpufreq_err("@%s: read PMIC_ADDR_VCORE_VOSEL_ON(0x%x), rdata = 0x%x\n", __func__, PMIC_ADDR_VCORE_VOSEL_ON, rdata);
...
static unsigned int get_cur_volt_extbuck(struct mt_cpu_dvfs *p)
{
    ...
    if (cpu_dvfs_is_extbuck_valid()) {
        do {
			...
			/*if (!fan53555_read_byte(fan53555_VDVFS11_CON13, &ret_val)) {
...
static int _mt_cpufreq_thermal_limited_verify(struct mt_cpu_dvfs *p, int new_opp_idx)
{
...
static unsigned int _mt_cpufreq_calc_new_opp_idx(struct mt_cpu_dvfs *p, int new_opp_idx)
{
    ...
    idx = _mt_cpufreq_thermal_limited_verify(p, new_opp_idx);
    ...
    if (p->dvfs_disable_by_early_suspend) {
				...
				new_opp_idx = CPU_DVFS_FREQ_OPP_MAX;
    ...
    if (p->dvfs_disable_by_suspend) {
				//new_opp_idx = p->idx_normal_max_opp;
				new_opp_idx = CPU_DVFS_FREQ_OPP_MAX;
        cpufreq_dbg("%s(): for suspend, idx = %d\n", __func__, new_opp_idx);
		}
    ...
    if (p->dvfs_disable_by_ptpod) {
        ...
        idx = _mt_cpufreq_get_idx_by_freq(p, CPU_DVFS_FREQ6, CPUFREQ_RELATION_L);
        if (idx != -1) {
            new_opp_idx = idx;
...
static void _mt_cpufreq_set(enum mt_cpu_dvfs_id id, int new_opp_idx)
{
	...
	cpufreq_dbg("@%s(): policy = %d, cur_freq = %d KHz, target_freq = %d KHz\n",
                __func__,
								policy->policy,
                cur_freq,
								target_freq
...
void mt_cpufreq_thermal_protect(unsigned int limited_power)
{
...
#ifdef CONFIG_CPU_FREQ
    {
        ...
        // correct opp idx will be calcualted in _mt_cpufreq_thermal_limited_verify()
        _mt_cpufreq_set(MT_CPU_DVFS_LITTLE, -1);
...
{
	cpufreq_write(FREQ_MTR_CTRL, (1 << RG_FQMTR_RST_BIT));
	dsb();
	cpufreq_write(FREQ_MTR_CTRL, 0);
}

INT32 freqmeter_kick(FREQMETER_CTRL *ctl)
{
	UINT32	reg;
	...
	reg = (//((ctl->divider & RG_FQMTR_CKDIV_MASK) << RG_FQMTR_CKDIV_BIT) |
	       ((ctl->ref_clk_sel & RG_FQMTR_FIXCLK_SEL_MASK) << RG_FQMTR_FIXCLK_SEL_BIT) | //26M
	       ((ctl->mon_sel & RG_FQMTR_MONCLK_SEL_MASK) << RG_FQMTR_MONCLK_SEL_BIT) | //0x19 => 25
	       ((ctl->mon_len_in_ref_clk & RG_FQMTR_WINDOW_MASK) << RG_FQMTR_WINDOW_BIT) | //0x400
	       (1 << RG_FQMTR_EN_BIT)); //<<15

	cpufreq_write(FREQ_MTR_CTRL, reg);
...
#define RG_FRMTR_WINDOW     0x400

INT32 freqmeter_getresult(FREQMETER_CTRL *ctl)
{
	UINT32	reg;
	INT32	status = FREQMETER_SUCCESS;
	...
	while (1) {
		udelay(10);
		reg = cpufreq_read(FREQ_MTR_DATA);
...
#define TEST_DBG_CTRL_BIT       (0)
#define TEST_DBG_CTRL_MASK      (0xff)
#define DBG_CTRL_MASK           (0xff)
#define INFRA_AO_DBG_CON0_MASK  (0x1f)
#define INFRA_AO_DBG_CON0_BIT   (0)
#define DBG_CTRL_BIT            (0)

int _mt_get_cpu_freq()
{
	...
	test_dbg_ctrl = cpufreq_read(TEST_DBG_CTRL_REG);
	CLR_FIELD(test_dbg_ctrl, TEST_DBG_CTRL);
	SET_FIELD(test_dbg_ctrl, (unsigned int)0x0, TEST_DBG_CTRL);
	cpufreq_write(TEST_DBG_CTRL_REG, 0x1);
	test_dbg_ctrl = cpufreq_read(TEST_DBG_CTRL_REG);
	...
	if (res != FREQMETER_SUCCESS) {
		//- check error code
		cpufreq_dbg("_mt_get_cpu_freq: freq meter kick fail!! \n");
		return -1;
	} else {
		//- polling mode
		...
		if (res != FREQMETER_SUCCESS) {
			//- check error code
			cpufreq_dbg("_mt_get_cpu_freq: freqmeter get result fail!! \n");
			return -1;
		} else {
			//- success
			return ((fqmtr.result_in_count * 26) / RG_FRMTR_WINDOW+1) * 2;
		}
	}
}
...
unsigned int dvfs_get_cpu_freq(enum mt_cpu_dvfs_id id)
{
	...
	//return _mt_cpufreq_get_cur_phy_freq(NULL);
}
...
void dvfs_disable_by_ptpod(int id)
{
    ...
    //little_freq_backup = _mt_cpufreq_get_cur_phy_freq(p);
	switch (id) {
	case PTP_CTRL_CPU:
		vcpu_backup = cpu_dvfs_get_cur_volt(p);
		mt_cpufreq_disable_by_ptpod(MT_CPU_DVFS_LITTLE);
		dvfs_set_cpu_volt(MT_CPU_DVFS_LITTLE, 131250);    // 1V
		break;
	case PTP_CTRL_ALL:
		vcpu_backup = cpu_dvfs_get_cur_volt(p);
		mt_cpufreq_disable_by_ptpod(MT_CPU_DVFS_LITTLE);
