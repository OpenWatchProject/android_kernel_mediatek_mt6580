		...
		void *buf, size_t len)
{
	...
	if((ch_id == DATA_CH_ID) || (ch_id == SDIO_AT_CHANNEL_NUM) || (ch_id == SDIO_AT2_CHANNEL_NUM)
			|| (ch_id == EXCP_CTRL_CH_ID) || (ch_id == EXCP_MSG_CH_ID) || (ch_id == AGPS_CH_ID)) {
...
int sdio_modem_ccmni_send_pkt(int md_id, int tx_ch, void *data)
{
	...
	if(data_len > (FIFO_SIZE - chars_in_fifo)){
		...
		if(ccmni_port->tx_state == CCMNI_TX_READY){
			//stop tx queue
			LOGPRT(LOG_INFO,  "ccmni (md_id:%d,tc_ch:%d) is stopping.\n", md_id, tx_ch);
...
static ssize_t dev_char_write(struct file *file, const char __user *buf, size_t count, loff_t *ppos)
{
	...
	if(modem->status != MD_OFF){
		...
		if ((port->index != (FLS_CH_ID-1)) && (count > (ONE_PACKET_MAX_SIZE - chars_in_fifo))){
			pr_info("[C2K]port%d too many(%d) data pending...flow ctrl(%d)\n",
