...
int ddp_dsi_build_cmdq(DISP_MODULE_ENUM module, void *cmdq_trigger_handle, CMDQ_STATE state)
{
	...
	DSI_T0_INS t0;
	DSI_RX_DATA_REG read_data0, read_data1, read_data2, read_data3;
	...
	else if(state == CMDQ_ESD_CHECK_CMP)
	{
		...
		for(i = 0; i < 3; i++)
		{
			...
			{
                    // read from slot
                    cmdqBackupReadSlot(hSlot, i * 4 + 0, &read_data0);
                    cmdqBackupReadSlot(hSlot, i * 4 + 1, &read_data1);
                    cmdqBackupReadSlot(hSlot, i * 4 + 2, &read_data2);
                    cmdqBackupReadSlot(hSlot, i * 4 + 3, &read_data3);
			}
			else
			{
			...
			// 0x21: dcs short read response(1 byte return)
			// 0x22: dcs short read response(2 byte return)
			unsigned char packet_type = read_data0.byte0;
