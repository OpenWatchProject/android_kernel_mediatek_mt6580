  ...
  if (idx == 0)
  {
      ...
      if ( (width >= 800) || ((width>=1280) && (height<=800)) )
      {
          ...
          DISP_REG_SET(handle,idx*DISP_RDMA_INDEX_OFFSET+ DISP_REG_RDMA_MEM_GMC_SETTING_0, 
              ultra_low_level|(pre_ultra_low_ofs<<8)|(ultra_high_ofs<<16)|(pre_ultra_high_ofs<<24));
          // Issue Reg threshold
          DISP_REG_SET(handle,idx*DISP_RDMA_INDEX_OFFSET+ DISP_REG_RDMA_MEM_GMC_SETTING_1, 128);
      ...
      else if ( (width >= 720) || ((width>=1280) && (height<=720)) ) // HD
      {
          // Issue Reg threshold
          DISP_REG_SET(handle,idx*DISP_RDMA_INDEX_OFFSET+ DISP_REG_RDMA_MEM_GMC_SETTING_1, 180);
          // Best ultra
          DISP_REG_SET(handle,idx*DISP_RDMA_INDEX_OFFSET+ DISP_REG_RDMA_MEM_GMC_SETTING_0, 0x0C011832);
          // Max ultra
          //DISP_REG_SET(handle,idx*DISP_RDMA_INDEX_OFFSET+ DISP_REG_RDMA_MEM_GMC_SETTING_0, 0x0C011880);
          if (mode == RDMA_MODE_DIRECT_LINK)
          {
              sodi_threshold = 75|(88<<10);//low:75	high:88 (direct link in HD)	
              fifo_valid_size = 74 + 1;
          }
          else // mode == RDMA_MODE_DECOUPLE
          {
              sodi_threshold = 75|(88<<10);//low:75 high:88 (decouple in HD)
              fifo_valid_size = 74 + 1;
              if (isidle) // home screen idle senario (300MHz), in future we should setting by different clock
              {
                  sodi_threshold = 75|(88<<10);//low:75 high:88 (decouple in HD)
      ...
      else if ( (width >= 540) || ((width>=960) && (height<=540)) ) // qHD
      {
          //DISP_REG_SET(handle,idx*DISP_RDMA_INDEX_OFFSET+ DISP_REG_RDMA_MEM_GMC_SETTING_0, );
          DISP_REG_SET(handle,idx*DISP_RDMA_INDEX_OFFSET+ DISP_REG_RDMA_MEM_GMC_SETTING_1, 213);
          // Best ultra
          DISP_REG_SET(handle,idx*DISP_RDMA_INDEX_OFFSET+ DISP_REG_RDMA_MEM_GMC_SETTING_0, 0x07010E1C);
          // Max ultra
          //DISP_REG_SET(handle,idx*DISP_RDMA_INDEX_OFFSET+ DISP_REG_RDMA_MEM_GMC_SETTING_0, 0x07010E80);
          if (mode == RDMA_MODE_DIRECT_LINK)
          {
              sodi_threshold = 42|(49<<10);//low:42	high:49 (direct link in qHD)
              fifo_valid_size = 42 + 1;
          }
          else // mode == RDMA_MODE_DECOUPLE
          {
              sodi_threshold = 42|(49<<10);//low:42 high:49 (decouple in qHD)
              fifo_valid_size = 42 + 1;

              if (isidle) // home screen idle senario (300MHz), in future we should setting by different clock
              {
                  sodi_threshold = 42|(49<<10);//low:21 high:153 (decouple in qHD when home screen idle senario)
                  fifo_valid_size = 32;
                  DISP_REG_SET(handle,idx*DISP_RDMA_INDEX_OFFSET+ DISP_REG_RDMA_MEM_GMC_SETTING_1, 192);
                  DISP_REG_SET(handle,idx*DISP_RDMA_INDEX_OFFSET+ DISP_REG_RDMA_MEM_GMC_SETTING_0, 0x07010E1C);
              }
          }
      }
      else if ( (width >= 480) || ((width>=640) && (height<=480)) ) // WVGA
      {
          // Issue Reg threshold
          DISP_REG_SET(handle,idx*DISP_RDMA_INDEX_OFFSET+ DISP_REG_RDMA_MEM_GMC_SETTING_1, 221);
          // Best ultra
          DISP_REG_SET(handle,idx*DISP_RDMA_INDEX_OFFSET+ DISP_REG_RDMA_MEM_GMC_SETTING_0, 0x06010816);
          // Max ultra
          //DISP_REG_SET(handle,idx*DISP_RDMA_INDEX_OFFSET+ DISP_REG_RDMA_MEM_GMC_SETTING_0, 0x0xxxxxxx);
          if (mode == RDMA_MODE_DIRECT_LINK)
          {
              sodi_threshold = 34|(44<<10);//low:34	high:44 (direct link in WVGA)
              fifo_valid_size = 32;
          }
          else // mode == RDMA_MODE_DECOUPLE
          {
              sodi_threshold = 34|(39<<10);//low:34 high:39 (decouple in WVGA)
              fifo_valid_size = 32;
              if (isidle) // home screen idle senario (300MHz), in future we should setting by different clock
              {
                  sodi_threshold = 17|(152<<10);//low:17 high:152 (decouple in WVGA when home screen idle senario)
                  fifo_valid_size = 32;
                  DISP_REG_SET(handle,idx*DISP_RDMA_INDEX_OFFSET+ DISP_REG_RDMA_MEM_GMC_SETTING_1, 196);
                  DISP_REG_SET(handle,idx*DISP_RDMA_INDEX_OFFSET+ DISP_REG_RDMA_MEM_GMC_SETTING_0, 0x05010A06);
              }
          }
      }
      else if ( (width >= 320) || ((width>=480) && (height<=320)) ) // HVGA
      {
          // Issue Reg threshold
          DISP_REG_SET(handle,idx*DISP_RDMA_INDEX_OFFSET+ DISP_REG_RDMA_MEM_GMC_SETTING_1, 221);
          // Best ultra
          ...
          if (mode == RDMA_MODE_DIRECT_LINK)
          {
              sodi_threshold = 34|(44<<10);//low:34	high:44 (direct link in HVGA)
              fifo_valid_size = 32;
          }
          else // mode == RDMA_MODE_DECOUPLE
          {
              sodi_threshold = 34|(39<<10);//low:34 high:39 (decouple in HVGA)
              fifo_valid_size = 32;
              if (isidle) // home screen idle senario (300MHz), in future we should setting by different clock
              {
                  sodi_threshold = 18|(21<<10);//low:17 high:152 (decouple in WVGA when home screen idle senario)
                  fifo_valid_size = 32;
                  DISP_REG_SET(handle,idx*DISP_RDMA_INDEX_OFFSET+ DISP_REG_RDMA_MEM_GMC_SETTING_1, 208);
                  DISP_REG_SET(handle,idx*DISP_RDMA_INDEX_OFFSET+ DISP_REG_RDMA_MEM_GMC_SETTING_0, 0x0201020C);
              }
          }
      }
      else 
