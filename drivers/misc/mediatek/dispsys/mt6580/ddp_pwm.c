...
static int disp_pwm_parse_triple(const char *cmd, unsigned long *offset, unsigned int *value, unsigned int *mask)
{
    int count = 0;
    char *next = (char*)cmd;
...
static void disp_pwm_set_reg(const char *cmd)
{
	const unsigned long reg_base = pwm_get_reg_base(DISP_PWM0);
	unsigned long offset;
    unsigned int value, mask;
	int count = disp_pwm_parse_triple(cmd, &offset, &value, &mask);
        if (count == 3) {
