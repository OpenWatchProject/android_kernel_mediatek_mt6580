 ...
 *     		CMDQ_CODE_WFE					: wait for event and clear		=> create via cmdqRecWait()
 *     		CMDQ_CODE_SET_TOKEN			: set event					=> create via cmdqRecSetEventToken()
 *     		CMDQ_CODE_WAIT_NO_CLEAR		: wait event, but don't clear it	=> create via cmdqRecWaitNoClear()
 *     		CMDQ_CODE_CLEAR_TOKEN			: clear event					=> create via cmdqRecClearEventToken()
 *     		CMDQ_CODE_PREFETCH_ENABLE		: enable prefetch marke			=> create via cmdqRecEnablePrefetch()
 *     		CMDQ_CODE_PREFETCH_DISABLE		: disable prefetch marker		=> create via cmdqRecDisablePrefetch()
