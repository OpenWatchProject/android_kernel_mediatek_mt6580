...
//extern function
//============================================================ //
extern kal_uint32 PMIC_IMM_GetOneChannelValue(mt6350_adc_ch_list_enum dwChannel, int deCount, int trimd);
...
static kal_int32 read_adc_v_i_sense(void *data)
{
    ...
    *(kal_int32*)(data) = 4202;
#else
    *(kal_int32*)(data) = PMIC_IMM_GetOneChannelValue(MT6350_AUX_ISENSE,*(kal_int32*)(data),1);
#endif
...
{
	*(kal_int32*)(data) = get_battery_plug_out_status();
...
extern void pmic_turn_on_clock(kal_uint32 enable);
#endif
...
static kal_int32 fgauge_get_low_battery_interrupt_status(void *data)
{
#if defined(CONFIG_MTK_HAFG_20)
	*(kal_int32*)(data) = is_lbat_int;
#else
	*(kal_int32*)(data) = KAL_FALSE;
#endif
...
static kal_int32 (* const bm_func[BATTERY_METER_CMD_NUMBER])(void *data)=
{
    ...
    ,fgauge_set_low_battery_interrupt
