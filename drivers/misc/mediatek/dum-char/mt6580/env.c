...
static int is_first_read_write = 0;
...
static int ENV_Self_Init(char *env_buf)
{
    ...
    if(is_first_read_write == 1)
    {
		...
		if(ret <0 ) // If Read fail, don't do Self Init this time
        {
...
int ENV_Nand_Read(char *env_buf)
{
    ...
    {
        printk("ENV_Nand_Read: wrong misc mtd number: %d!!!\n", misc_mtd_number);
        return -1;
    }
    mtd = get_mtd_device(NULL, misc_mtd_number);
    if (!mtd) {
        printk("ENV_Nand_Read: get device info fail!!!\n");
        put_mtd_device(mtd);
        return -1;
    }
    ...
    if (result) {
        printk("ENV_Nand_Read: read env data fail, result = %d !!!\n", result);
        put_mtd_device(mtd);
        return -1;
    }
    put_mtd_device(mtd);
...
int ENV_Nand_Write(char *env_buf)
{
    ...
    if(result)
    {
        printk("ENV_Nand_Write: erase device failed");
        put_mtd_device(mtd);
        return -1;
    }
    ...
    if (result) {
        printk("ENV_Nand_Write: Write env data fail, result = %d !!!\n", result);
        put_mtd_device(mtd);
        return -1;
    }
