...
static void _DrvFwCtrlMsg28xxWriteEFlashStart(u16 nStartAddr, u8 *pFirstData, u32 nBlockSize, u16 nPageNum, EmemType_e eEmemType)
{
    ...
    // Write mode
    RegSetLByteValue(0x1606, 0x80);
    
    if (eEmemType == EMEM_INFO)
    {
        RegSet16BitValueOn(0x1606, BIT11);
        RegSet16BitValueOn(0x1604, BIT0);
    }
...
static void _DrvFwCtrlMsg28xxWriteEFlashDoWrite(u8 *pBufferData, u32 nLength)
{
    u32 i;
...
static void _DrvFwCtrlMsg28xxEraseEmem(EmemType_e eEmemType)
{
    ...
    else if (eEmemType == EMEM_INFO) // 2KB
    {
        ...
        // Set info block password
        RegSet16BitValue(0x1616, 0x55AA);
        ...
        for (nInfoAddr = 0x20; nInfoAddr <= MSG28XX_EMEM_INFO_MAX_ADDR; nInfoAddr += 0x20)
        {
            ...
            while (1) // Wait erase done
            {
                ...
                if ((nTimeOut ++) > 10)
                {
                    ...
                    // Clear info block password
                    RegSet16BitValue(0x1616, 0xA55A);
...
{
    u32 i, j;
    u32 nPageNum = 0, nLength = 0, nIndex = 0;
    u32 nRetryTime = 0;
    u8  nRegData = 0;
    ...
    if (eEmemType == EMEM_MAIN) // Program main block
    {
        ...
        for (i = 0; i < nPageNum; i ++) 
        {
            if (i == 0)
            {
                ...
                _DrvFwCtrlMsg28xxWriteEFlashStart(nIndex, &szFirstData[0], MSG28XX_FIRMWARE_MAIN_BLOCK_SIZE*1024, nPageNum, EMEM_MAIN);
            ...
            _DrvFwCtrlMsg28xxWriteEFlashDoWrite(&szBufferData[0], nLength);
    ...
    else if (eEmemType == EMEM_INFO) // Program info block
    {
        ...
        for (i = 1; i < nPageNum; i ++) // Skip firt page
        {
            if (i == 1)
            {
                // Read first data 4 bytes
                ...
                _DrvFwCtrlMsg28xxWriteEFlashStart(nIndex, &szFirstData[0], MSG28XX_FIRMWARE_INFO_BLOCK_SIZE*1024, nPageNum, EMEM_INFO);
        ...
        while ((nRegData & BIT3) != BIT3)
        {
            ...
            if (nRetryTime ++ > 100)
            {
                DBG("Info block can't wait write to done.\n");
                break;
            }
        }
