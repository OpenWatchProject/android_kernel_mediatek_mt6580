...
static	void tpd_power_switch(s32 state)
{
	int ret = 0;
	switch (state) {
		case POWER_ON:
...
static	int tpd_power_init(int retry)
{
...
__probe_retry:
	...
	if(retval<0)
	{
		if(retry_count++ < 3)
		{
			printk("mtk_tpd[FTS] Read I2C error! try again , retry count:( %d ).\n",retry_count);
			tpd_power_switch(POWER_OFF);
			msleep(20);
			goto __probe_retry;
		}
		else
	...
	*/			
	...
	for (i = 0; i < data->touch_point; i++) 
	{
		...
		if (data->au8_touch_event[i]== 0 || data->au8_touch_event[i] == 2)
		{
			...
			input_report_abs(tpd->dev, ABS_MT_POSITION_X,data->au16_y[i]);
			input_report_abs(tpd->dev, ABS_MT_POSITION_Y,TPD_RES_X - data->au16_x[i]); 
			#elif MTK_LCM_PHYSICAL_ROTATION == 180
			input_report_abs(tpd->dev, ABS_MT_POSITION_X,TPD_RES_X - data->au16_x[i]);
			input_report_abs(tpd->dev, ABS_MT_POSITION_Y,TPD_RES_Y - data->au16_y[i]); 
			#elif MTK_LCM_PHYSICAL_ROTATION == 90
			temp = x;
			x = TPD_RES_Y -y;
			y = temp;
			input_report_abs(tpd->dev, ABS_MT_POSITION_X,TPD_RES_Y - data->au16_y[i]);
		...
		#else
		{
			if (tpd_touchinfo(&cinfo, &pinfo)) 
			{
				...
				if(point_num >0) 
				{
				    ...
				    {
						#if MTK_LCM_PHYSICAL_ROTATION== 270
							tpd_down(cinfo.y[i], TPD_RES_X - cinfo.x[i], cinfo.id[i]);
						#elif MTK_LCM_PHYSICAL_ROTATION == 180
							tpd_down(TPD_RES_X - cinfo.x[i], TPD_RES_Y - cinfo.y[i], cinfo.id[i]);
						#elif MTK_LCM_PHYSICAL_ROTATION == 90
							tpd_down(TPD_RES_Y - cinfo.y[i], cinfo.x[i], cinfo.id[i]);
						#else
	    			...
	    			{
						#if MTK_LCM_PHYSICAL_ROTATION== 270
							tpd_up(cinfo.y[0], TPD_RES_X - cinfo.x[0], cinfo.id[0]);
						#elif MTK_LCM_PHYSICAL_ROTATION == 180
							tpd_up(TPD_RES_X - cinfo.x[0], TPD_RES_Y - cinfo.y[0], cinfo.id[0]);
						#elif MTK_LCM_PHYSICAL_ROTATION == 90
							tpd_up(TPD_RES_Y - cinfo.y[0], cinfo.x[0], cinfo.id[0]);
						#else
