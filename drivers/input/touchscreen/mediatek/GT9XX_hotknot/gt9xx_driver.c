...
static int gesture_function_enable = 0;
...
#if defined(BT_CHARGE)
#include <linux/workqueue.h>
static void bt_charge_eint_interrupt_handler(void);
static void bt_charge_eint_work(struct work_struct *work);

struct work_struct	bt_chare_eint_work_queue;
static u8 bt_charge_eint_state = 0;
#endif
...
static s8 gtp_i2c_test(struct i2c_client *client)
{
    ...
    while (retry++ < 1)
    {
        ret = i2c_read_bytes_non_dma(client, GTP_REG_HW_INFO, (u8 *)&hw_info, sizeof(hw_info));
...
//#ifdef VELOCITY_CUSTOM
	...
	input_set_capability(tpd->dev, EV_KEY, KEY_G);
	input_set_capability(tpd->dev, EV_KEY, KEY_H);
	input_set_capability(tpd->dev, EV_KEY, KEY_M);
	input_set_capability(tpd->dev, EV_KEY, KEY_O);
//	input_set_capability(tpd->dev, EV_KEY, KEY_Q);
	...
	input_set_capability(tpd->dev, EV_KEY, KEY_Y);
	...
	INIT_WORK(&bt_chare_eint_work_queue, bt_charge_eint_work);
	
	// set INT mode
    mt_set_gpio_mode(GPIO_BT_EXT_EINT_PIN, GPIO_BT_EXT_EINT_PIN_M_EINT);
    mt_set_gpio_dir(GPIO_BT_EXT_EINT_PIN, GPIO_DIR_IN);

	//set enable pin
	mt_set_gpio_mode(GPIO_BT_EXT_CHARGE_EN_PIN, GPIO_BT_EXT_EINT_PIN_M_GPIO);
    mt_set_gpio_dir(GPIO_BT_EXT_CHARGE_EN_PIN, GPIO_DIR_OUT);
	mt_set_gpio_out(GPIO_BT_EXT_CHARGE_EN_PIN, GPIO_OUT_ZERO);  

	printk("bt charge eint registration\n");
	mt_eint_registration(CUST_EINT_BT_EXT_NUM, CUST_EINTF_TRIGGER_LOW, bt_charge_eint_interrupt_handler, 0);
	
   
	 mt_eint_unmask(CUST_EINT_BT_EXT_NUM);
	 bt_charge_eint_state =  CUST_EINTF_TRIGGER_LOW;
#endif
...
static void bt_charge_eint_interrupt_handler(void)
{
	printk(KERN_ERR"\n##########bt_charge_eint_interrupt_handler #########\n");

	 mt_eint_mask(CUST_EINT_BT_EXT_NUM);

	 schedule_work(&bt_chare_eint_work_queue);
}
...
static void bt_charge_eint_work(struct work_struct *work)
{
	 printk("bt_charge_eint_work,bt_charge_eint_state=%d\n",bt_charge_eint_state);

	if(bt_charge_eint_state == CUST_EINTF_TRIGGER_LOW)
	{
		printk("########bt charge low state######\n");
		mt_eint_set_polarity(CUST_EINT_BT_EXT_NUM, 1);
		bt_charge_eint_state = CUST_EINTF_TRIGGER_HIGH;

		//BT start charge
		mt_set_gpio_out(GPIO_BT_EXT_CHARGE_EN_PIN, GPIO_OUT_ONE);  
		
		
	}
	else if(bt_charge_eint_state == CUST_EINTF_TRIGGER_HIGH)
	{
		printk("########bt charge high state######\n");
		mt_eint_set_polarity(CUST_EINT_BT_EXT_NUM, 0);
		bt_charge_eint_state = CUST_EINTF_TRIGGER_LOW;
	...
	{
		printk("########bt charge low state error######\n");
	}
    ...
    printk(KERN_ERR"bt_charge_eint_work F9 send\n\n");
	mt_eint_unmask(CUST_EINT_BT_EXT_NUM);
}
...
{
    printk(KERN_ERR"\n#####input_report_gesture_key,keycode=0x%x#####\n\n" ,keycode);
    switch ( keycode )
    {
    case 0xAA:
    ...
    case 0xBA:
          ...
          input_sync(tpd->dev);
          printk(KERN_ERR"Up received\n\n");
        break;
	case 'a':
		 input_report_key(tpd->dev, KEY_A, 1);
          input_sync(tpd->dev);
          input_report_key(tpd->dev, KEY_A, 0);
          input_sync(tpd->dev);
           printk(KERN_ERR"a received\n\n");	
		break;
	case 'b':
		 input_report_key(tpd->dev, KEY_B, 1);
          input_sync(tpd->dev);
          input_report_key(tpd->dev, KEY_B, 0);
          input_sync(tpd->dev);
           printk(KERN_ERR"b received\n\n");	
		break;
	case 'c':
		 input_report_key(tpd->dev, KEY_C, 1);
          ...
          input_sync(tpd->dev);
           printk(KERN_ERR"c received\n\n");	
	break;
	case 'd':
		 input_report_key(tpd->dev, KEY_D, 1);
          input_sync(tpd->dev);
          input_report_key(tpd->dev, KEY_D, 0);
          input_sync(tpd->dev);
           printk(KERN_ERR"d received\n\n");	
	break;
	case 'e':
      ...
      input_sync(tpd->dev);
      printk(KERN_ERR"e received\n\n");	
	break;
	case 'g':
	  input_report_key(tpd->dev, KEY_G, 1);
      input_sync(tpd->dev);
      input_report_key(tpd->dev, KEY_G, 0);
      input_sync(tpd->dev);
       printk(KERN_ERR"g received\n\n");	
	break;
	case 'h':
	...
	case 'w':
      ...
      input_sync(tpd->dev);
      printk(KERN_ERR"w received\n\n");	
	break;
	case 'y':
	  input_report_key(tpd->dev, KEY_Y, 1);
      input_sync(tpd->dev);
      input_report_key(tpd->dev, KEY_Y, 0);
      input_sync(tpd->dev);
      printk(KERN_ERR"y received\n\n");	
	break;
	case 'z':
...
static int touch_event_handler(void *unused)
{
   ...
   u8  rqst_data[3] = {(u8)(GTP_REG_RQST >> 8), (u8)(GTP_REG_RQST & 0xFF), 0};
	...
	#if GTP_GESTURE_WAKEUP
		 printk(KERN_ERR"############ last_gesture_status=0X%x ##########\n",last_gesture_status);
        if (DOZE_ENABLED == doze_status)
        {
            ret = gtp_i2c_read(i2c_client_point, doze_buf, 3);
             printk(KERN_ERR"0x814B = 0x%02X\n", doze_buf[2]);
            if (ret > 0)
            {               
                    ...
                    )
                {
					...
					last_gesture_status = doze_buf[2];
                //    doze_status = DOZE_WAKEUP;
					input_report_gesture_key(doze_buf[2]);
				#if 0
...
#if GTP_GESTURE_WAKEUP
	printk(KERN_ERR"gesture_function_enable=%d\n",gesture_function_enable);
	if(gesture_function_enable)
	{
		printk(KERN_ERR"gesture_function_enable 1\n");
    	ret = gtp_enter_doze(i2c_client_point);
    }
	else
	{
		printk(KERN_ERR"gesture_function_enable 0\n");
		mt_eint_mask(CUST_EINT_TOUCH_PANEL_NUM);
